...
bool is_BaikalM_project(void)
{
	int prj_id = get_project();

	if (prj_id == 24035 || prj_id == 24345) {
		return 1;
	} else {
		return 0;
	}
...
bool is_GreenLand_project(void)
{
	int prj_id = get_project();
...
#ifdef OPLUS_FEATURE_DISPLAY_APOLLO
	if (!(oplus_apollo_unsupported()) || setbrightness_force_file()) {
		mutex_lock(&led_dat->led_access);
