...
#include "oplus25711_nt37706a_fhdp_dsi_vdo_144hz_dphy_boe.h"
            ...
            char *src, int s_len)
{
    char dest[512] = {0,};
    int len = 0;
    len = iris_conver_one_panel_cmd(dest, src, s_len < 512 ? s_len : 512);
}

static int iris_dcs_write(struct mipi_dsi_device *dsi, const void *data, size_t len)
{
    int valid = 0;
    bool change = false;
    int ret = 0;
    ...
    if (data == NULL)
        return -INVALID;
    addr = (char *)data;
    if (iris_is_chip_supported() && iris_is_pt_mode(false) && len > 0) {
...
static void iris_panel_init(void)
{
...
static void lcm_dcs_write(struct lcm *ctx, const void *data, size_t len)
{
    ...
    addr = (char *)data;
#if defined(CONFIG_PXLW_IRIS)
    ret = iris_dcs_write(dsi, data, len);
#else
    if ((int)*addr < 0xB0)
        ret = mipi_dsi_dcs_write_buffer(dsi, data, len);
    else
...
#define PANEL_CLK               (720)
#define DATA_RATE               (1440)
#define DYN_PLL_CLOCK           (701)
#define DYN_DATA_RATE           (1402)
#define DYN_DATA_RATE_KHZ       (1401600)
#define DYN_HFP                 (204)
...
#define HFP_144HZ               (114)
#define HFP                     (224)
#define HBP                     (20)
#define HSA                     (4)
#define VFP_60HZ                (3076)
#define VFP_90HZ                (1116)
...
static struct mtk_panel_params ext_params_60Hz = {
    ...
    .dyn = {
        .switch_en = 1,
        .pll_clk = DYN_PLL_CLOCK,
        .data_rate = DYN_DATA_RATE,
        .data_rate_khz = DYN_DATA_RATE_KHZ,
        .hfp = DYN_HFP,
        .vfp = VFP_60HZ,
        .vsa = VSA,
        .vbp = VBP,
...
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
    ...
    if (last_brightness >= 1088 && oplus_display_brightness < 1088) {
        if (iris_is_chip_supported() && iris_is_pt_mode(false)) {
            iris_seed_cmd(dsi, cb, handle, bl_tb1, ARRAY_SIZE(bl_tb1));
            iris_seed_cmd(dsi, cb, handle, bl_tb2, ARRAY_SIZE(bl_tb2));
            iris_seed_cmd(dsi, cb, handle, bl_tb3_1, ARRAY_SIZE(bl_tb3_1));
        } else {
        ...
        if (iris_is_chip_supported() && iris_is_pt_mode(false)) {
            iris_seed_cmd(dsi, cb, handle, bl_tb1, ARRAY_SIZE(bl_tb1));
            iris_seed_cmd(dsi, cb, handle, bl_tb2, ARRAY_SIZE(bl_tb2));
            iris_seed_cmd(dsi, cb, handle, bl_tb3_2, ARRAY_SIZE(bl_tb3_2));
        } else {
            cb(dsi, handle, bl_tb1, ARRAY_SIZE(bl_tb1));
            cb(dsi, handle, bl_tb2, ARRAY_SIZE(bl_tb2));
...
#if defined(CONFIG_PXLW_IRIS)
        if (iris_is_chip_supported() && iris_is_pt_mode(false)) {
            iris_seed_cmd(dsi, cb, handle, lcm_setbrightness_normal[i].para_list, lcm_setbrightness_normal[i].count);
        } else {
...
#if defined(CONFIG_PXLW_IRIS)
                if (iris_is_chip_supported() && iris_is_pt_mode(false)) {
                    iris_seed_cmd(dsi, cb, handle, lcm_finger_lhbm_on_setting_T0[i].para_list, lcm_finger_lhbm_on_setting_T0[i].count);
                } else {
...
#if defined(CONFIG_PXLW_IRIS)
                if (iris_is_chip_supported() && iris_is_pt_mode(false)) {
                    iris_seed_cmd(dsi, cb, handle, lcm_lhbm_off_setbrightness_normal_T0[i].para_list, lcm_lhbm_off_setbrightness_normal_T0[i].count);
                } else {
...
#if defined(CONFIG_PXLW_IRIS)
                if (iris_is_chip_supported() && iris_is_pt_mode(false)) {
                    iris_seed_cmd(dsi, cb, handle, lcm_finger_lhbm_on_setting_EVT[i].para_list, lcm_finger_lhbm_on_setting_EVT[i].count);
                } else {
...
#if defined(CONFIG_PXLW_IRIS)
                if (iris_is_chip_supported() && iris_is_pt_mode(false)) {
                    iris_seed_cmd(dsi, cb, handle, lcm_lhbm_off_setbrightness_normal_EVT[i].para_list, lcm_lhbm_off_setbrightness_normal_EVT[i].count);
                } else {
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
    ...
    register_device_proc("lcd", "BOE_NT37706A", "BOE_CASIOM");
    ctx->hbm_en = false;
    oplus_max_normal_brightness = MAX_NORMAL_BRIGHTNESS;
    oplus_enhance_mipi_strength = 3;
    oplus_ofp_init(dev);
...
static const struct of_device_id lcm_of_match[] = {
    { .compatible = "oplus25711,nt37706a,fhdp,dsi,vdo,144hz,dphy,boe", },
    { }
};
...
static struct mipi_dsi_driver lcm_driver = {
    ...
    .remove = lcm_remove,
    .driver = {
        .name = "oplus25711_nt37706a_fhdp_dsi_vdo_144hz_dphy_boe",
        .owner = THIS_MODULE,
