...
#define MT6319_PMIC_DIG_WPK_KEY		0x3a8
#define MT6319_PMIC_DIG_WPK_KEY_H	0x3a9
#define MT6319_PMIC_TMA_KEY		0x39f
#define MT6319_PMIC_TMA_KEY_H		0x3a0
#define MT6319_TOP2_ELR2		0x134
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
static void mt6319_strength_change(struct pmic_core *core)
{
	unsigned int reg_val;
	int ret;

	ret = of_property_read_u32(core->dev->of_node, "mt6319-strength", &reg_val);
	if (!ret) {
		/* Disable write protection key */
		regmap_write(core->regmap, MT6319_PMIC_DIG_WPK_KEY, 0x15);
		regmap_write(core->regmap, MT6319_PMIC_DIG_WPK_KEY_H, 0x63);
		regmap_write(core->regmap, MT6319_PMIC_TMA_KEY, 0xea);
		regmap_write(core->regmap, MT6319_PMIC_TMA_KEY_H, 0x9c);
		/* Modify RG_OCTL_SCLK */
		regmap_write(core->regmap, MT6319_TOP2_ELR2, reg_val);
		/* Enable write protection key */
		regmap_write(core->regmap, MT6319_PMIC_DIG_WPK_KEY, 0);
		regmap_write(core->regmap, MT6319_PMIC_DIG_WPK_KEY_H, 0);
		regmap_write(core->regmap, MT6319_PMIC_TMA_KEY, 0);
		regmap_write(core->regmap, MT6319_PMIC_TMA_KEY_H, 0);
	}
}
#endif
