...
static int mt6685_strength_change(struct device *dev, struct regmap *regmap)
{
	unsigned int drv, reg_val;
	int ret;

	ret = of_property_read_u32(dev->of_node, "mt6685-sclk-strength", &reg_val);
	if (!ret) {
		ret = regmap_read(regmap, 0x130, &drv);
		if (ret) {
			pr_info("Failed to read mt6685 sclk reg!\n");
			return ret;
		}
		drv &= ~0x3f;
		drv |= reg_val;
		regmap_write(regmap, 0x130, drv);
	}
	ret = of_property_read_u32(dev->of_node, "mt6685-sdata-strength", &reg_val);
	if (!ret) {
		ret = regmap_read(regmap, 0x131, &drv);
		if (ret) {
			pr_info("Failed to read mt6685 sdata reg!\n");
			return ret;
		}
		drv &= ~0x3f;
		drv |= reg_val;
		regmap_write(regmap, 0x131, drv);
	}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	mt6685_strength_change(&sdev->dev, regmap);
#endif
