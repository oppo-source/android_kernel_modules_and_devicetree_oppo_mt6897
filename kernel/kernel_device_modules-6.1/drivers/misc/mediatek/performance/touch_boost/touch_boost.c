...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_CEILING_FREE)
static int ceiling_free_enable = 0;
#endif
...
static ssize_t perfmgr_gas_for_ta_proc_show(struct file *file,
		char __user *ubuf, size_t count, loff_t *ppos)
{
	int n = 0;
	char buffer[64];
...
static ssize_t perfmgr_gas_threshold_for_ta_proc_write(struct file *filp,
		const char *ubuf, size_t cnt, loff_t *data)
{
	char buf[64];
	int value;
...
static ssize_t perfmgr_gas_threshold_for_ta_proc_show(struct file *file,
		char __user *ubuf, size_t count, loff_t *ppos)
{
	int n = 0;
	char buffer[64];
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_CEILING_FREE)
static ssize_t perfmgr_ceiling_free_enable_proc_write(struct file *filp,
		const char *ubuf, size_t cnt, loff_t *data)
{
	char buf[64];
	int value;
	...
	ceiling_free_enable = value;

	return cnt;
}

static ssize_t perfmgr_ceiling_free_enable_proc_show(struct file *file,
		char __user *ubuf, size_t count, loff_t *ppos)
{
	int n = 0;
	char buffer[64];
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_CEILING_FREE)
PROC_FOPS_RW(ceiling_free_enable);
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_CEILING_FREE)
		PROC_ENTRY(ceiling_free_enable),
#endif
	};
