...
KERNEL_MAKE_DEPENDENCIES += $(shell find vendor/oplus/kernel -name .git -prune -o -type f | sort)
KERNEL_MAKE_DEPENDENCIES += $(shell find vendor/oplus/sensor -name .git -prune -o -type f | sort)
KERNEL_MAKE_DEPENDENCIES += $(shell find vendor/oplus/secure -name .git -prune -o -type f | sort)
...
$(KERNEL_ZIMAGE_OUT): PRIVATE_BAZEL_EXPORT_ENV := BAZEL_DO_NOT_DETECT_CPP_TOOLCHAIN=1 DEFCONFIG_OVERLAYS="$(KERNEL_DEFCONFIG_OVERLAYS)" KERNEL_VERSION=$(LINUX_KERNEL_VERSION) OPLUS_PLATFORM_INFO=$(COMPILE_PLATFORM) OPLUS_FEATURES="$(PRIVATE_OPLUS_FEATURES)" KERNEL_BUILD_VARIANT="$(TARGET_BUILD_VARIANT)"
