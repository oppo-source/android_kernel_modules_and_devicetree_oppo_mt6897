...
struct mtk_afe_i2s_priv {
	...
	/* determine whether iv latch point needs to be modified */
	unsigned int iv_latch_point;
	/* determine whether iv latch point needs to be modified for sia */
	unsigned int iv_latch_point_sia_enable;
#endif
};
...
#else
		if (i2s_priv->iv_latch_point) {
			regmap_update_bits(afe->regmap, ETDM_IN4_CON1,
					REG_INITIAL_POINT_MASK_SFT,
					i2s_priv->iv_latch_point << REG_INITIAL_POINT_SFT);
		} else {
			regmap_update_bits(afe->regmap, ETDM_IN4_CON1,
					REG_INITIAL_POINT_MASK_SFT,
					0x3 << REG_INITIAL_POINT_SFT);
...
#ifdef OPLUS_ARCH_EXTENDS
		/* determine whether iv latch point needs to be modified for sia */
		if (i2s_priv->iv_latch_point_sia_enable) {
			regmap_update_bits(afe->regmap, ETDM_IN4_CON2,
					   REG_SDATA_DELAY_0P5T_EN_MASK_SFT,
					   0x1 << REG_SDATA_DELAY_0P5T_EN_SFT);
		}
#endif /* OPLUS_ARCH_EXTENDS end */
...
#ifdef OPLUS_ARCH_EXTENDS
	/* determine whether iv latch point needs to be modified */
	ret = of_property_read_u32(afe->dev->of_node, "etdm-iv-latch-point", &iv_latch_point);
	if (ret) {
		dev_info(afe->dev, "%s() failed to read etdm-iv-latch-point\n", __func__);
		iv_latch_point = 0;
	}
	i2sin4_priv->iv_latch_point = iv_latch_point;
	dev_info(afe->dev, "%s() etdm-iv-latch-point: %d\n", __func__, iv_latch_point);
	/* determine whether iv latch point needs to be modified for sia */
	ret = of_property_read_u32(afe->dev->of_node, "etdm-iv-latch-point-sia-enable", &iv_latch_point_sia_enable);
	if (ret) {
		dev_info(afe->dev, "%s() failed to read etdm-iv-latch-point-sia-enable \n", __func__);
		iv_latch_point_sia_enable = 0;
	}
	i2sin4_priv->iv_latch_point_sia_enable = iv_latch_point_sia_enable;
	dev_info(afe->dev, "%s() etdm-iv-latch-point-sia-enable : %d\n", __func__, iv_latch_point_sia_enable);
#endif
