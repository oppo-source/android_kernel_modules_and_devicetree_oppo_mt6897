				  ...
				  struct snd_soc_dai *dai)
{
	...
	case MT6897_DAI_I2S_IN0:
		...
		if (afe_priv->iv_latch_point_modify) {
			regmap_update_bits(afe->regmap, ETDM_IN0_CON1,
				   REG_INITIAL_POINT_MASK_SFT,
...
#ifdef OPLUS_ARCH_EXTENDS
	/* determine whether iv latch point needs to be modified */
	ret = of_property_read_u32(afe->dev->of_node, "iv-latch-point-modify", &afe_priv->iv_latch_point_modify);
	if (ret) {
		dev_info(afe->dev, "%s() failed to read iv-latch-point-modify\n", __func__);
		afe_priv->iv_latch_point_modify = 0;
	}
	dev_info(afe->dev, "%s() iv-latch-point-modify: %d\n", __func__, afe_priv->iv_latch_point_modify);
#endif
