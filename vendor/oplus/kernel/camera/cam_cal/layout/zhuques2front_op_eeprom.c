...
#define PFX "CAM_CAL_ZHUQUES2FRONT"
...
static unsigned int do_single_lsc_zhuques2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_zhuques2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_zhuques2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_zhuques2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x0175010A, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000C00, 0x0000074C, do_single_lsc_zhuques2},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_zhuques2}, //Start address, block size is useless
		{0x00000001, 0x00001400, 0x000005ED, do_pdaf_zhuques2},
		{0x00000000, 0x00000000, 0x00000000, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_zhuques2}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT zhuques2front_op_eeprom = {
	.name = "zhuques2front_op_eeprom",
	.check_layout_function = layout_check,
	.read_function = Common_read_region,
	.layout = &cal_layout_table,
	.sensor_id = ZHUQUES2FRONT_SENSOR_ID,
	.i2c_write_id = 0xA8,
	.max_size = 0x8000,
	.enable_preload = 1,
	.preload_size = 0x8000,
...
#define ZHUQUES2FRONT_HVBIN_PDAF_PROC1_SIZE  (496)
#define ZHUQUES2FRONT_HVBIN_PDAF_PROC2_SIZE  (1004)
#define ZHUQUES2FRONT_HVBIN_PDAF_PROC1_ADDR  (0x1400)
#define ZHUQUES2FRONT_HVBIN_PDAF_PROC2_ADDR  (0x1600)

unsigned int do_pdaf_zhuques2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	pCamCalData->PDAF.Size_of_PDAF = ZHUQUES2FRONT_HVBIN_PDAF_PROC1_SIZE + ZHUQUES2FRONT_HVBIN_PDAF_PROC2_SIZE;
...
//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			ZHUQUES2FRONT_HVBIN_PDAF_PROC1_ADDR + ZHUQUES2FRONT_HVBIN_PDAF_PROC1_SIZE,
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			ZHUQUES2FRONT_HVBIN_PDAF_PROC2_ADDR + ZHUQUES2FRONT_HVBIN_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			ZHUQUES2FRONT_HVBIN_PDAF_PROC2_ADDR, ZHUQUES2FRONT_HVBIN_PDAF_PROC2_SIZE,
			(unsigned char *)&pCamCalData->PDAF.Data[ZHUQUES2FRONT_HVBIN_PDAF_PROC1_SIZE]);
	if (read_data_size > 0)
		err = CAM_CAL_ERR_NO_ERR;
