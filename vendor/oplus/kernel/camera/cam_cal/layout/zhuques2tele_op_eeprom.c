...
#define PFX "CAM_CAL_ZHUQUES2TELE"
...
static unsigned int do_single_lsc_zhuques2tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_zhuques2tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_zhuques2tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_zhuques2tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_ois_zhuques2tele(unsigned char *eeprom_preload,
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x00c6010D, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000B00, 0x0000074C, do_single_lsc_zhuques2tele},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_zhuques2tele}, //Start address, block size is useless
		{0x00000001, 0x00001300, 0x00000A3A, do_pdaf_zhuques2tele},
		{0x00000000, 0x00000000, 0x00000000, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_zhuques2tele}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT zhuques2tele_op_eeprom = {
	.name = "zhuques2tele_op_eeprom",
	.check_layout_function = layout_check,
	.read_function = Common_read_region,
	.layout = &cal_layout_table,
	.sensor_id = ZHUQUES2TELE_SENSOR_ID,
	.i2c_write_id = 0xA0,
	.max_size = 0x8000,
	.enable_preload = 1,
	.preload_size = 0x8000,
	.has_stored_data = 1,
	.parsing_ois_cal_data_from_preload = do_ois_zhuques2tele,
};


#define ZHUQUES2TELE_HVBIN_PDAF_PROC1_SIZE  (496)
#define ZHUQUES2TELE_HVBIN_PDAF_PROC2_SIZE  (1004)
#define ZHUQUES2TELE_HVBIN_PDAF_PROC1_ADDR  (0x1300)
#define ZHUQUES2TELE_HVBIN_PDAF_PROC2_ADDR  (0x1500)

unsigned int do_pdaf_zhuques2tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	pCamCalData->PDAF.Size_of_PDAF = ZHUQUES2TELE_HVBIN_PDAF_PROC1_SIZE + ZHUQUES2TELE_HVBIN_PDAF_PROC2_SIZE;
...
//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			ZHUQUES2TELE_HVBIN_PDAF_PROC1_ADDR + ZHUQUES2TELE_HVBIN_PDAF_PROC1_SIZE,
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			ZHUQUES2TELE_HVBIN_PDAF_PROC2_ADDR + ZHUQUES2TELE_HVBIN_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			ZHUQUES2TELE_HVBIN_PDAF_PROC2_ADDR, ZHUQUES2TELE_HVBIN_PDAF_PROC2_SIZE,
			(unsigned char *)&pCamCalData->PDAF.Data[ZHUQUES2TELE_HVBIN_PDAF_PROC1_SIZE]);
	if (read_data_size > 0)
		err = CAM_CAL_ERR_NO_ERR;
