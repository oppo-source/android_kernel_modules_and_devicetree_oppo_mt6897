...
#define PFX "CAM_CAL_CAYMANCFRONT"
...
#define DEBUG_CALIBRATION_LOAD 1
#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)

static unsigned int do_single_lsc_caymancfront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_caymancfront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_caymancfront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_caymancfront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static unsigned int layout_check_caymancfront(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01c40055, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000530, 0x0000074C, do_single_lsc_caymancfront},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_caymancfront}, //Start address, block size is useless
		{0x00000001, 0x00000000, 0x00000000, do_pdaf_caymancfront},
		{0x00000000, 0x00000000, 0x00000000, do_stereo_data},
		{0x00000001, 0x00000000, 0x00002000, do_dump_all},
		{0x00000001, 0x00000001, 0x00000004, do_lens_id_caymancfront}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT caymancfront_op_eeprom = {
	.name = "caymancfront_op_eeprom",
	.check_layout_function = layout_check_caymancfront,
...
static struct STRUCT_CAM_CAL_CONFIG_STRUCT *cam_cal_config = &caymancfront_op_eeprom;

/*
unsigned int do_pdaf_caymancfront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
...
#if 0
#define CAYMANCFRONT_HVBIN_PDAF_PROC1_SIZE  (0x1BA0-0x17D0) //976      0x3D0
#define CAYMANCFRONT_HVBIN_PDAF_PROC2_SIZE  (0x2208-0x1BB0) //1624     2600  0xA28
#define CAYMANCFRONT_HVBIN_PDAF_PROC1_ADDR  (0x17D0)
#define CAYMANCFRONT_HVBIN_PDAF_PROC2_ADDR  (0x1BB0)

//partial PD
#define CAYMANCFRONT_PARTIAL_PD_PROC1_SIZE  (0x3590-0x33A0) //496       0X1F0
#define CAYMANCFRONT_PARTIAL_PD_PROC2_SIZE  (0x398C-0x35A0) //1004      1500   0x5DC
#define CAYMANCFRONT_PARTIAL_PD_PROC1_ADDR  (0x33A0)
#define CAYMANCFRONT_PARTIAL_PD_PROC2_ADDR  (0x35A0)
unsigned int do_pdaf_caymancfront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	return err;
	...
	LOG_INF("======================PDAF Data==================\n");
	LOG_INF("[%s] QPD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, CAYMANCFRONT_HVBIN_PDAF_PROC1_ADDR, CAYMANCFRONT_HVBIN_PDAF_PROC1_SIZE);
	LOG_INF("[%s] QPD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, CAYMANCFRONT_HVBIN_PDAF_PROC2_ADDR, CAYMANCFRONT_HVBIN_PDAF_PROC2_SIZE);
	LOG_INF("[%s] partial PD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, CAYMANCFRONT_PARTIAL_PD_PROC1_ADDR, CAYMANCFRONT_PARTIAL_PD_PROC1_SIZE);
	LOG_INF("[%s] partial PD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, CAYMANCFRONT_PARTIAL_PD_PROC2_ADDR, CAYMANCFRONT_PARTIAL_PD_PROC2_SIZE);
...
// partial PD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			CAYMANCFRONT_PARTIAL_PD_PROC1_ADDR + CAYMANCFRONT_PARTIAL_PD_PROC1_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
		LOG_INF("[%s] partial PD proc1 unvalid\n", __FUNCTION__);
	} else {
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				CAYMANCFRONT_PARTIAL_PD_PROC1_ADDR, CAYMANCFRONT_PARTIAL_PD_PROC1_SIZE,
				(unsigned char *)&pCamCalData->PDAF.Data[bios]);
		if (read_data_size > 0) {
			LOG_INF("[%s] partial PD proc1 First five %x, %x, %x, %x, %x\n",
				__FUNCTION__,
			...
			bios += CAYMANCFRONT_PARTIAL_PD_PROC1_SIZE;
			partial_pd_proc1_flag = true;
			LOG_INF("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
	...
	// partial PD proc2
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				CAYMANCFRONT_PARTIAL_PD_PROC2_ADDR + CAYMANCFRONT_PARTIAL_PD_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
			LOG_INF("[%s]  proc2 unvalid\n", __FUNCTION__);
		} else {
			read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
					CAYMANCFRONT_PARTIAL_PD_PROC2_ADDR, CAYMANCFRONT_PARTIAL_PD_PROC2_SIZE,
					(unsigned char *)&pCamCalData->PDAF.Data[bios]);
			if (read_data_size > 0) {
				LOG_INF("[%s] partial PD proc2 First five %x, %x, %x, %x, %x\n",
					__FUNCTION__,
					pCamCalData->PDAF.Data[bios],
					pCamCalData->PDAF.Data[bios + 1],
					pCamCalData->PDAF.Data[bios + 2],
					pCamCalData->PDAF.Data[bios + 3],
					pCamCalData->PDAF.Data[bios + 4]);
				bios += CAYMANCFRONT_PARTIAL_PD_PROC2_SIZE;
	...
	if(partial_pd_proc1_flag && partial_pd_proc2_flag) {
		...
		LOG_INF("[%s] partial PD eeprom error",__FUNCTION__);
	}
	err = CAM_CAL_ERR_NO_ERR;
...
// QPD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			CAYMANCFRONT_HVBIN_PDAF_PROC1_ADDR + CAYMANCFRONT_HVBIN_PDAF_PROC1_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
		LOG_INF("[%s] QPD proc1 unvalid\n", __FUNCTION__);
	} else {
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				CAYMANCFRONT_HVBIN_PDAF_PROC1_ADDR, CAYMANCFRONT_HVBIN_PDAF_PROC1_SIZE,
				(unsigned char *)&pCamCalData->PDAF.Data[bios]);
		if (read_data_size > 0) {
			LOG_INF("[%s] QDP proc1 First five %x, %x, %x, %x, %x\n",
				__FUNCTION__,
				pCamCalData->PDAF.Data[bios],
				pCamCalData->PDAF.Data[bios + 1],
				pCamCalData->PDAF.Data[bios + 2],
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += CAYMANCFRONT_HVBIN_PDAF_PROC1_SIZE;
			qpd_proc1_flag = true;
			LOG_INF("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
...
// QPD proc2
	if(qpd_proc1_flag) {
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				CAYMANCFRONT_HVBIN_PDAF_PROC2_ADDR + CAYMANCFRONT_HVBIN_PDAF_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
			LOG_INF("[%s] QPD proc2 unvalid\n", __FUNCTION__);
		}
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				CAYMANCFRONT_HVBIN_PDAF_PROC2_ADDR, CAYMANCFRONT_HVBIN_PDAF_PROC2_SIZE,
				(unsigned char *)&pCamCalData->PDAF.Data[bios]);
		if (read_data_size > 0) {
			LOG_INF("[%s] QDP proc2 First five %x, %x, %x, %x, %x\n",
				__FUNCTION__,
				pCamCalData->PDAF.Data[bios],
				pCamCalData->PDAF.Data[bios + 1],
				pCamCalData->PDAF.Data[bios + 2],
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += CAYMANCFRONT_HVBIN_PDAF_PROC2_SIZE;
			qpd_proc2_flag = true;
			LOG_INF("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
	...
	if(qpd_proc1_flag && qpd_proc2_flag) {
		...
		bios = pCamCalData->PDAF.Size_of_PDAF;
		LOG_INF("[%s] QPD eeprom error",__FUNCTION__);
	}
	if(pCamCalData->PDAF.Size_of_PDAF == 0) {
		err =  CamCalReturnErr[pCamCalData->Command];
	}

	LOG_INF("[%s] pCamCalData->PDAF.Size_of_PDAF= %d  0x%x\n", __FUNCTION__,
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (table_size > 0) {
		...
		LOG_INF("u4Offset=%d u4Length=%d", start_addr, table_size);
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	LOG_INF("S2aBitEn=0x%02x", pCamCalData->Single2A.S2aBitEn);
	...
	if (0x1 & AWBAFConfig) {
		...
		if (read_data_size > 0)	{
			LOG_INF( "Read CalValue OK\n");
			printk("awb Calibration OK tanhao");
			rgCalValue  = CalValue & 0xFFFF;
			bgCalValue = (CalValue >> 16) & 0xFFFF;
			LOG_INF("Light source calibration 5100K value R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			printk("caymancLight source calibration 5100K value R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
		...
		if (read_data_size > 0)	{
			...
			if (CalR > CalG)
				...
				if (CalG > CalB)
					...
					tempMax = CalB;
			LOG_INF("UnitR:%d, UnitG:%d, UnitB:%d, New Unit Max=%d",
					CalR, CalG, CalB, tempMax);
			printk("caymanc5000k UnitR:%d, UnitG:%d, UnitB:%d, New Unit Max=%d",
					CalR, CalG, CalB, tempMax);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read CalGain Failed\n");
			printk("caymanc5000k fail UnitR:%d, UnitG:%d, UnitB:%d, New Unit Max=%d",
					CalR, CalG, CalB, tempMax);
			show_cmd_error_log(pCamCalData->Command);
		}
		if (CalGain != 0x0000000000000000 &&
			...
			LOG_INF("There are something wrong on EEPROM, plz contact module vendor!!\n");
			LOG_INF("Unit R=%d G=%d B=%d!!\n", CalR, CalG, CalB);
		}
		...
		if (read_data_size > 0)	{
			...
			if (FacR > FacG)
				...
				if (FacG > FacB)
					...
					tempMax = FacB;
			LOG_INF("GoldenR:%d, GoldenG:%d, GoldenB:%d, New Golden Max=%d",
					FacR, FacG, FacB, tempMax);
			printk("caymancGoldenR:%d, GoldenG:%d, GoldenB:%d, New Golden Max=%d",
					FacR, FacG, FacB, tempMax);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read FacGain Failed\n");
			printk("caymancfail GoldenR:%d, GoldenG:%d, GoldenB:%d, New Golden Max=%d",
					FacR, FacG, FacB, tempMax);
			show_cmd_error_log(pCamCalData->Command);
		}
		...
		#ifdef DEBUG_CALIBRATION_LOAD
		LOG_INF("======================AWB CAM_CAL==================\n");
		LOG_INF("AWB Calibration @5100K\n");
		LOG_INF("[CalGain] = 0x%lld\n", CalGain);
		LOG_INF("[FacGain] = 0x%lld\n", FacGain);
		...
		if (read_data_size > 0)	{
			...
			rgCalValue  = CalValue & 0xFFFF;
			bgCalValue = (CalValue >> 16) & 0xFFFF;
			LOG_INF("Light source calibration value 3100 R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			printk("caymancLight source calibration value 3100 R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read CalGain Failed\n");
			printk("caymancfail Light source calibration value 3100 R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			show_cmd_error_log(pCamCalData->Command);
		}
		/* AWB Unit Gain (3100K) */
		awb_offset = 0x0034;
		...
		if (read_data_size > 0)	{
			...
			CalG  = ((CalGr + CalGb) + 1) >> 1;
			CalB  = (CalGain >> 48) & 0xFFFF;
			LOG_INF("CalR:%d, CalB:%d",CalR, CalB);
			CalR  = CalR * rgCalValue / 1000;
			CalB  = CalB * bgCalValue / 1000;
			if (CalR > CalG)
				/* R > G */
...
static unsigned int layout_check_caymancfront(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID)
{
	...
	if (cam_cal_config->sensor_id == sensorID)
		LOG_INF("%s sensor_id matched\n", cam_cal_config->name);
	else {
		LOG_INF("%s sensor_id not matched\n", cam_cal_config->name);
		return result;
	}

	if (read_data_region(pdata, (u8 *)&check_id, header_offset, 4) != 4) {
		LOG_INF("header_id read failed\n");
		return result;
	}

	if (check_id == 0x01c40055 || check_id == 0x01C3012B || check_id == 0x00510055) {	// hearder id on OTP guide
		LOG_INF("header_id matched 0x%08x\n", check_id);
		printk("caymancheader_id matched 0x%08x\n", check_id);
		result = CAM_CAL_ERR_NO_ERR;
	} else{
		LOG_INF("header_id not matched 0x%08x\n", check_id);
		printk("caymancheader_id not matched 0x%08x\n", check_id);
	}
	return result;
}
