...
#define PFX "CAM_CAL_BAIKALMFRONT2"
...
static unsigned int do_single_lsc_baikalmfront2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_baikalmfront2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_baikalmfront2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_baikalmfront2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x0401007F, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000000, 0x00000002, do_module_version},
		{0x00000001, 0x00000000, 0x00000011, do_part_number},
		{0x00000001, 0x00000600, 0x0000074C, do_single_lsc_baikalmfront2},
		{0x00000001, 0x00000020, 0x0000000E, do_2a_gain_baikalmfront2}, //Start address, block size is useless
		{0x00000000, 0x00001300, 0x000005DC, do_pdaf_baikalmfront2},
		{0x00000000, 0x00000FAE, 0x00000550, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_baikalmfront2}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT baikalmfront2_op_eeprom = {
	.name = "baikalmfront2_op_eeprom",
...
#define BAIKALMFRONT2_HVBIN_PDAF_PROC1_SIZE  (496)
#define BAIKALMFRONT2_HVBIN_PDAF_PROC2_SIZE  (1004)
#define BAIKALMFRONT2_HVBIN_PDAF_PROC1_ADDR  (0x1300)
#define BAIKALMFRONT2_HVBIN_PDAF_PROC2_ADDR  (0x1500)
static unsigned int do_pdaf_baikalmfront2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	struct STRUCT_CAM_CAL_DATA_STRUCT *pCamCalData =
	...
	pCamCalData->PDAF.Size_of_PDAF = BAIKALMFRONT2_HVBIN_PDAF_PROC1_SIZE + BAIKALMFRONT2_HVBIN_PDAF_PROC2_SIZE;
	debug_log("PDAF start_addr =%x table_size=%d\n", start_addr, block_size);

//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			BAIKALMFRONT2_HVBIN_PDAF_PROC1_ADDR + BAIKALMFRONT2_HVBIN_PDAF_PROC1_SIZE,
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			BAIKALMFRONT2_HVBIN_PDAF_PROC2_ADDR + BAIKALMFRONT2_HVBIN_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			BAIKALMFRONT2_HVBIN_PDAF_PROC2_ADDR, BAIKALMFRONT2_HVBIN_PDAF_PROC2_SIZE,
			(unsigned char *)&pCamCalData->PDAF.Data[BAIKALMFRONT2_HVBIN_PDAF_PROC1_SIZE]);
	if (read_data_size > 0)
		err = CAM_CAL_ERR_NO_ERR;
