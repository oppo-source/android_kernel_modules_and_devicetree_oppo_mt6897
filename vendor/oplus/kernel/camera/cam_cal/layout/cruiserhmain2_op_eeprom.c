...
#define PFX "CAM_CAL_CRUISERHMAIN2"
...
static unsigned int do_single_lsc_cruiserhmain2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_cruiserhmain2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_cruiserhmain2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_cruiserhmain2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x016B0071, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000000, 0x00000000, do_module_version},
		{0x00000001, 0x00000000, 0x00000002, do_part_number},
		{0x00000001, 0x00001000, 0x0000074C, do_single_lsc_cruiserhmain2},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_cruiserhmain2}, //Start address, block size is useless
		{0x00000001, 0x00001750, 0x00000A3A, do_pdaf_cruiserhmain2},
		{0x00000000, 0x00000FAE, 0x00000550, do_stereo_data},
		{0x00000001, 0x00000000, 0x00004000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_cruiserhmain2}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT cruiserhmain2_op_eeprom = {
	.name = "cruiserhmain2_op_eeprom",
...
/*
unsigned int do_pdaf_cruiserhmain2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
...
#define CRUISERHMAIN2_HVBIN_PDAF_PROC1_SIZE  (0x1940-0x1750) //496      0x1F0
#define CRUISERHMAIN2_HVBIN_PDAF_PROC2_SIZE  (0x1D3C-0x1950) //1004     0X3EC
#define CRUISERHMAIN2_HVBIN_PDAF_PROC1_ADDR  (0x1750)
#define CRUISERHMAIN2_HVBIN_PDAF_PROC2_ADDR  (0x1950)

//partial PD
#define CRUISERHMAIN2_PARTIAL_PD_PROC1_SIZE  (0x1F30-0x1D40) //496       0X1F0
#define CRUISERHMAIN2_PARTIAL_PD_PROC2_SIZE  (0x232C-0x1F40) //1004      1500   0x5DC
#define CRUISERHMAIN2_PARTIAL_PD_PROC1_ADDR  (0x1D40)
#define CRUISERHMAIN2_PARTIAL_PD_PROC2_ADDR  (0x1F40)
unsigned int do_pdaf_cruiserhmain2(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	struct STRUCT_CAM_CAL_DATA_STRUCT *pCamCalData =
	...
	debug_log("======================PDAF Data==================\n");
	debug_log("[%s] QPD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, CRUISERHMAIN2_HVBIN_PDAF_PROC1_ADDR, CRUISERHMAIN2_HVBIN_PDAF_PROC1_SIZE);
	debug_log("[%s] QPD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, CRUISERHMAIN2_HVBIN_PDAF_PROC2_ADDR, CRUISERHMAIN2_HVBIN_PDAF_PROC2_SIZE);
	debug_log("[%s] partial PD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, CRUISERHMAIN2_PARTIAL_PD_PROC1_ADDR, CRUISERHMAIN2_PARTIAL_PD_PROC1_SIZE);
	debug_log("[%s] partial PD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, CRUISERHMAIN2_PARTIAL_PD_PROC2_ADDR, CRUISERHMAIN2_PARTIAL_PD_PROC2_SIZE);
...
// QPD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			CRUISERHMAIN2_HVBIN_PDAF_PROC1_ADDR + CRUISERHMAIN2_HVBIN_PDAF_PROC1_SIZE,
	...
	if(isValid != 1) {
		...
		if (read_data_size > 0) {
			...
			bios += CRUISERHMAIN2_HVBIN_PDAF_PROC1_SIZE;
			qpd_proc1_flag = true;
...
// QPD proc2
	if(qpd_proc1_flag) {
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				CRUISERHMAIN2_HVBIN_PDAF_PROC2_ADDR + CRUISERHMAIN2_HVBIN_PDAF_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
		...
		if (read_data_size > 0) {
			debug_log("[%s] QDP proc2 First five %x, %x, %x, %x, %x\n",
				...
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += CRUISERHMAIN2_HVBIN_PDAF_PROC2_SIZE;
			qpd_proc2_flag = true;
			debug_log("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
...
// partial PD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			CRUISERHMAIN2_PARTIAL_PD_PROC1_ADDR + CRUISERHMAIN2_PARTIAL_PD_PROC1_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
		...
		if (read_data_size > 0) {
			debug_log("[%s] partial PD proc1 First five %x, %x, %x, %x, %x\n",
				...
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += CRUISERHMAIN2_PARTIAL_PD_PROC1_SIZE;
	...
	// partial PD proc2
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				CRUISERHMAIN2_PARTIAL_PD_PROC2_ADDR + CRUISERHMAIN2_PARTIAL_PD_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
			...
			if (read_data_size > 0) {
				debug_log("[%s] partial PD proc2 First five %x, %x, %x, %x, %x\n",
					...
					pCamCalData->PDAF.Data[bios + 3],
					pCamCalData->PDAF.Data[bios + 4]);
				bios += CRUISERHMAIN2_PARTIAL_PD_PROC2_SIZE;
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (0x1 & AWBAFConfig) {
		...
		if (read_data_size > 0)	{
			...
			debug_log("CalR:%d, CalG:%d, CalB:%d",CalR, CalG, CalB);
			CalR  = (CalR * rgCalValue + 500) / 1000;
			CalB  = (CalB * bgCalValue + 500) / 1000;
			if (CalR > CalG)
				/* R > G */
				if (CalR > CalB)
					tempMax = CalR;
