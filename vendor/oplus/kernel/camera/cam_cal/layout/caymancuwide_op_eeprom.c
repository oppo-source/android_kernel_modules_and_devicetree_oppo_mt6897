...
#define PFX "CAM_CAL_CAYMANCUWIDE"
...
static unsigned int do_single_lsc_caymancuwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_caymancuwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_caymancuwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static unsigned int layout_check_caymancuwide(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x0065009A, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000530, 0x0000074C, do_single_lsc_caymancuwide},
		{0x00000001, 0x00000020, 0x00000010, do_2a_gain_caymancuwide},  //Start address, block size is useless
		...
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_caymancuwide}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT caymancuwide_op_eeprom = {
	.name = "caymancuwide_op_eeprom",
	.check_layout_function = layout_check_caymancuwide,
...
static struct STRUCT_CAM_CAL_CONFIG_STRUCT *cam_cal_config = &caymancuwide_op_eeprom;
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (block_size != CAM_CAL_SINGLE_LSC_SIZE)
		LOG_INF("block_size(%d) is not match (%d)\n",
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (0x1 & AWBAFConfig) {
		...
		LOG_INF("[CalGain] = 0x%llx\n", CalGain);
		LOG_INF("[FacGain] = 0x%llx\n", FacGain);
...
static unsigned int layout_check_caymancuwide(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID)
{
	...
	if (check_id == 0x016B012B || check_id == 0x016d009a || check_id ==0x0065009a) {	// hearder id on OTP guide
		...
		printk("caymancheader_id wide matched 0x%08x\n", check_id);
		result = CAM_CAL_ERR_NO_ERR;
	} else{
		LOG_INF("header_id not matched 0x%08x\n", check_id);
		printk("caymancheader_id wide not matched 0x%08x\n", check_id);
	}
	return result;
}
