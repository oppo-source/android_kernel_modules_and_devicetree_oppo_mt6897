...
static unsigned int do_single_lsc_luxl5uwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_luxl5uwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_luxl5uwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_luxl5uwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x016F0131, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000B00, 0x0000074C, do_single_lsc_luxl5uwide},
		{0x00000001, 0x00000020, 0x0000000E, do_2a_gain_luxl5uwide}, //Start address, block size is useless
		{0x00000000, 0x00001300, 0x000005DC, do_pdaf_luxl5uwide},
		{0x00000000, 0x00000FAE, 0x00000550, do_stereo_data},
		{0x00000001, 0x00000000, 0x00002000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_luxl5uwide}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT luxl5uwide_op_eeprom = {
	.name = "luxl5uwide_op_eeprom",
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (0x1 & AWBAFConfig) {
		...
		pr_debug("[CalGain] = %llu\n", CalGain);
		pr_debug("[FacGain] = %llu\n", FacGain);
...
#define LUXL5UWIDE_HVBIN_PDAF_PROC1_SIZE  (496)
#define LUXL5UWIDE_HVBIN_PDAF_PROC2_SIZE  (1004)
#define LUXL5UWIDE_HVBIN_PDAF_PROC1_ADDR  (0x1300)
#define LUXL5UWIDE_HVBIN_PDAF_PROC2_ADDR  (0x1500)
static unsigned int do_pdaf_luxl5uwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	struct STRUCT_CAM_CAL_DATA_STRUCT *pCamCalData =
	...
	pCamCalData->PDAF.Size_of_PDAF = LUXL5UWIDE_HVBIN_PDAF_PROC1_SIZE + LUXL5UWIDE_HVBIN_PDAF_PROC2_SIZE;
	debug_log("PDAF start_addr =%x table_size=%d\n", start_addr, block_size);

//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			LUXL5UWIDE_HVBIN_PDAF_PROC1_ADDR + LUXL5UWIDE_HVBIN_PDAF_PROC1_SIZE,
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			LUXL5UWIDE_HVBIN_PDAF_PROC2_ADDR + LUXL5UWIDE_HVBIN_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			LUXL5UWIDE_HVBIN_PDAF_PROC2_ADDR, LUXL5UWIDE_HVBIN_PDAF_PROC2_SIZE,
			(unsigned char *)&pCamCalData->PDAF.Data[LUXL5UWIDE_HVBIN_PDAF_PROC1_SIZE]);
	if (read_data_size > 0)
		err = CAM_CAL_ERR_NO_ERR;
