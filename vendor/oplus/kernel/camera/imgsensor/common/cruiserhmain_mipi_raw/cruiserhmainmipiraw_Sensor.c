 ...
 *============================================================================
 ****************************************************************************/
#include "cruiserhmainmipiraw_Sensor.h"
//GC50F6
#define SENSOR_NAME  SENSOR_DRVNAME_CRUISERHMAIN_MIPI_RAW

#define CRUISERHMAIN_EEPROM_ADDR        (0xA0)
#define CRUISERHMAIN_EERPOM_MAX_OFFSET  (0x4000)
//#define OPLUS_CAMERA_COMMON_DATA_LENGTH (40)

#define PFX "cruiserhmain_camera_sensor"
...
#define CRUISERHMAIN_STEREO_MW_START_ADDR  (0x2980)
#define CRUISERHMAIN_AESYNC_START_ADDR     (0x2F90)

#define  OTP_PDC_IS_VALID_VAL     (0x01)
#define  OTP_PDC_VALID_ADDR       (0x1AED)
#define  XTC_SENSOR_ADDR          (0x0600)
#define  XTC_SENSOR_LENGTH  	  (490)
#define  XTC_SENSOR_LENGTH_PART1  (294)
#define  XTC_SENSOR_LENGTH_PART2  (196)

static u8 pdc_is_valid = 0;

#define CRUISERHMAIN_IMGSENSOR_ID   (0x50F6)

#define CRUISERHMAIN_UNIQUE_SENSOR_ID_ADDR    (0x7000)  //?????
#define CRUISERHMAIN_UNIQUE_SENSOR_ID_LENGTH  (16)
// static BYTE cruiserhmain_unique_id[CRUISERHMAIN_UNIQUE_SENSOR_ID_LENGTH] = { 0 };
static struct oplus_eeprom_info_struct  oplus_eeprom_info = {0};

static kal_uint8 otp_data_checksum[CRUISERHMAIN_EERPOM_MAX_OFFSET] = {0};
static int get_sensor_temperature(void *arg);
#define MAX_BURST_LEN  (2048)
static u8 * msg_buf = NULL;
static void set_group_hold(void *arg, u8 en);
static u16 get_gain2reg(u32 gain);
//static int cruiserhmain_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain_get_eeprom_comdata(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt);
//static void cruiserhmain_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter);
static int cruiserhmain_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void cruiserhmain_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length);
static void cruiserhmain_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt, u16 frame_length);
static int cruiserhmain_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int cruiserhmain_i2c_burst_wr_regs_u8(struct subdrv_ctx *ctx, u16 * list, u32 len);
static int adapter_i2c_burst_wr_regs_u8(struct subdrv_ctx * ctx,
		u16 addr, u16 *list, u32 len);
//static int cruiserhmain_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
//static int cruiserhmain_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
//static void cruiserhmain_write_frame_length(struct subdrv_ctx *ctx, u32 fll);
// static int cruiserhmain_get_unique_sensorid(struct subdrv_ctx *ctx, u8 *para, u32 *len);
//static int cruiserhmain_get_cloud_otp_info(struct subdrv_ctx *ctx, u8 *para, u32 *len);
// static int cruiserhmain_get_pdafblock_info(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void cruiserhmain_get_sensor_cali(void* arg);
static void cruiserhmain_set_sensor_cali(void *arg);
static int cruiserhmain_get_readout_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static bool cruiserhmain_set_long_exposure(struct subdrv_ctx *ctx);
/* STRUCT */
static void update_CTLE(struct subdrv_ctx *ctx);
#define WIDEC1_CTLE_LEVEL 1
#define WIDEC1_CTLE_EQBW 3

static struct eeprom_map_info cruiserhmain_eeprom_info[] = {
	...
	{ EEPROM_META_STEREO_MW_MAIN_DATA, CRUISERHMAIN_STEREO_MW_START_ADDR, 0xFFFF, 0xFFFF, CALI_DATA_SLAVE_LENGTH, false },
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, cruiserhmain_set_test_pattern},
//	{SENSOR_FEATURE_SEAMLESS_SWITCH, cruiserhmain_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, cruiserhmain_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, cruiserhmain_get_eeprom_comdata},
	{SENSOR_FEATURE_SET_SENSOR_OTP, cruiserhmain_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, cruiserhmain_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, cruiserhmain_get_otp_checksum_data},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, cruiserhmain_get_min_shutter_by_scenario_adapter},
//	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, cruiserhmain_streaming_suspend},
	{SENSOR_FEATURE_SET_ESHUTTER, cruiserhmain_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, cruiserhmain_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_HDR_SHUTTER, cruiserhmain_set_hdr_tri_shutter2},
	{SENSOR_FEATURE_SET_HDR_TRI_SHUTTER, cruiserhmain_set_hdr_tri_shutter3},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, cruiserhmain_set_multi_shutter_frame_length_ctrl},
//	{SENSOR_FEATURE_SET_STREAMING_RESUME, cruiserhmain_streaming_resume},
	// {SENSOR_FEATURE_GET_UNIQUE_SENSORID, cruiserhmain_get_unique_sensorid},
	// {SENSOR_FEATURE_GET_CLOUD_OTP_INFO, cruiserhmain_get_cloud_otp_info},
	// {SENSOR_FEATURE_GET_PDAF_INFO, cruiserhmain_get_pdafblock_info},
	{SENSOR_FEATURE_GET_READOUT_BY_SCENARIO, cruiserhmain_get_readout_by_scenario},
};

static struct eeprom_info_struct eeprom_info[] = {
	{
		.header_id = 0x01C50136,
		.addr_header_id = 0x00000006,
		.i2c_write_id = CRUISERHMAIN_EEPROM_ADDR,
		.pdc_support = TRUE,
...
static struct subdrv_mode_struct mode_struct[] = {
    {//M00-4096x3072-PD-992x760-HighGain-30fps.txt
		...
		.mode_setting_table = cruiserhmain_preview_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain_preview_setting),
	...
	{//M00-4096x3072-PD-992x760-HighGain-30fps.txt
		.frame_desc = frame_desc_prev_cap,
		.num_entries = ARRAY_SIZE(frame_desc_prev_cap),
		.mode_setting_table = cruiserhmain_capture_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain_capture_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{//M01-4096x2304-PD-992x576-HighGain-30fps.txt
		...
		.mode_setting_table = cruiserhmain_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {//M02-4096x2304-PD-992x576-NoralGain-60fps.txt
		...
		.mode_setting_table = cruiserhmain_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {//M03-4096x2048-PD-992x512-HighGain-30fps.txt
		...
		.mode_setting_table = cruiserhmain_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {//M05-3200x2400-PD-800x600-HighGain-24fps.txt
		...
		.mode_setting_table = cruiserhmain_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {//M06-1792x1344-PD-448x336-HighGain-24fps.txt
		...
		.mode_setting_table = cruiserhmain_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = cruiserhmain_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{//M03-4096x2048-PD-992x512-HighGain-30fps.txt
		...
		.mode_setting_table = cruiserhmain_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain_custom4_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = CRUISERHMAIN_SENSOR_ID,
	...
	.ana_gain_type = 4,
	.ana_gain_step = 1,
//	.ana_gain_table = cruiserhmain_ana_gain_table,
//	.ana_gain_table_size = sizeof(cruiserhmain_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 1,
	...
	.g_cali = cruiserhmain_get_sensor_cali,
	.s_gph = set_group_hold,
	.s_cali = cruiserhmain_set_sensor_cali,
	...
	.init_setting_table = cruiserhmain_init_setting,
	.init_setting_len = ARRAY_SIZE(cruiserhmain_init_setting),
	.mode = mode_struct,
...
struct subdrv_entry cruiserhmain_mipi_raw_entry = {
	.name = "cruiserhmain_mipi_raw",
	.id = CRUISERHMAIN_SENSOR_ID,
	.pw_seq = pw_seq,
...
static unsigned int read_cruiserhmain_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != cruiserhmain_eeprom_info[meta_id].meta)
		return -1;

	if (size != cruiserhmain_eeprom_info[meta_id].size)
		return -1;

	addr = cruiserhmain_eeprom_info[meta_id].start;
	readsize = cruiserhmain_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, CRUISERHMAIN_EEPROM_ADDR >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, CRUISERHMAIN_EEPROM_ADDR >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, CRUISERHMAIN_EEPROM_ADDR >> 1, reg, (CRUISERHMAIN_EEPROM_ADDR & 0xFE) | 0x01);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, CRUISERHMAIN_EEPROM_ADDR >> 1, reg, CRUISERHMAIN_EEPROM_ADDR & 0xFE);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if (((pStereodata->uSensorId == CRUISERHMAIN_SENSOR_ID))
            && (data_length == CALI_DATA_SLAVE_LENGTH)
            && (data_base == CRUISERHMAIN_STEREO_MW_START_ADDR)) {
        ...
        } else if (((pStereodata->uSensorId == CRUISERHMAIN_SENSOR_ID))
            && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == CRUISERHMAIN_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN_AESYNC_START_ADDR),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN_AESYNC_START_ADDR+1),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN_AESYNC_START_ADDR+2),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN_AESYNC_START_ADDR+3),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN_AESYNC_START_ADDR+4),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN_AESYNC_START_ADDR+5),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN_AESYNC_START_ADDR+6),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN_AESYNC_START_ADDR+7));
        ...
        LOG_INF("cruiserhmain write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int cruiserhmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	read_cruiserhmain_eeprom_info(ctx, EEPROM_META_STEREO_MW_MAIN_DATA,
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, CRUISERHMAIN_EEPROM_ADDR >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "cruiserhmain read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, sizeof(otp_data_checksum));
	DRV_LOGE(ctx, "cruiserhmain read_otp_info end\n");
}
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			*sensor_id = (subdrv_i2c_rd_u8(ctx, addr_h) << 8) |
			...
			if (*sensor_id == CRUISERHMAIN_IMGSENSOR_ID) {
...
static int open(struct subdrv_ctx *ctx)
{
	...
	// // software reset
	// subdrv_i2c_wr_regs_u8(ctx, cruiserhmain_soft_reset, ARRAY_SIZE(cruiserhmain_soft_reset));
	// msleep(1);

	//sensor_init(ctx);
	cruiserhmain_i2c_burst_wr_regs_u8(ctx, ctx->s_ctx.init_setting_table, ctx->s_ctx.init_setting_len);
...
void cruiserhmain_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
...
int cruiserhmain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64 *feature_data = (u64 *) para;
	cruiserhmain_get_min_shutter_by_scenario(ctx,
...
static void cruiserhmain_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt, u16 frame_length)
{
	...
	if(exp_cnt == 1) {  //force to 1exp func
		cruiserhmain_set_shutter_frame_length_convert(ctx, shutters[0], frame_length);
		return ;
	}
}
...
static int cruiserhmain_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	cruiserhmain_set_multi_shutter_frame_length(ctx, (u64 *)(*feature_data),
		(u64) (*(feature_data + 1)), (u64) (*(feature_data + 2)));
	return 0;
}

static void cruiserhmain_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt)
{
	int i = 0;
	u64 values[3] = {0};
	...
	cruiserhmain_set_multi_shutter_frame_length(ctx, values, exp_cnt, 0);
}
...
static int cruiserhmain_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	cruiserhmain_set_hdr_tri_shutter(ctx, feature_data, 2);
	return 0;
}
...
static int cruiserhmain_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	cruiserhmain_set_hdr_tri_shutter(ctx, feature_data, 3);
	return 0;
}
...
void cruiserhmain_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length)
{
	...
	if (cruiserhmain_set_long_exposure(ctx) == 0) { // Long exposure does not require setup framelength
		/* write framelength */
...
static int cruiserhmain_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	cruiserhmain_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], ((u64*)para)[1]);
	return 0;
}

//static void cruiserhmain_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter)
//{
//    cruiserhmain_set_shutter_frame_length_convert(ctx, shutter, 0);
//}
static int cruiserhmain_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	cruiserhmain_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], 0);
	return 0;
}
void cruiserhmain_set_dummy(struct subdrv_ctx *ctx)
{
}
...
{
	for (int scenario_id = 0; scenario_id < ctx->s_ctx.sensor_mode_num; ++scenario_id){
		if (ctx->s_ctx.sensor_id == CRUISERHMAIN_SENSOR_ID) {
