 ...
 *============================================================================
 ****************************************************************************/
#include "luxl5uwidemipiraw_Sensor.h"

#define LUXL5UWIDE_EEPROM_READ_ID	0xA3
#define LUXL5UWIDE_EEPROM_WRITE_ID   0xA2
#define LUXL5UWIDE_MAX_OFFSET		0x4000
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "luxl5uwide_camera_sensor"
...
static int luxl5uwide_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int luxl5uwide_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void luxl5uwide_write_discontinuity_mode(struct subdrv_ctx *ctx, enum SENSOR_SCENARIO_ID_ENUM scenario_id);
static int luxl5uwide_get_eeprom_4cell_info(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int luxl5uwide_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int luxl5uwide_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int luxl5uwide_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int luxl5uwide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int luxl5uwide_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static void luxl5uwide_write_preview_setting(struct subdrv_ctx *ctx);
static void luxl5uwide_write_normal_video_setting(struct subdrv_ctx *ctx);
static void luxl5uwide_write_hs_video_setting(struct subdrv_ctx *ctx);
#if 0
static void luxl5uwide_write_custom1_setting(struct subdrv_ctx *ctx);
static void luxl5uwide_write_custom2_setting(struct subdrv_ctx *ctx);
#endif
static int luxl5uwide_control(struct subdrv_ctx *ctx, enum SENSOR_SCENARIO_ID_ENUM scenario_id,
				MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window, MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data);
static void luxl5uwide_sensor_init(struct subdrv_ctx *ctx);
static void luxl5uwide_write_frame_length(struct subdrv_ctx *ctx, u32 fll);
static int open(struct subdrv_ctx *ctx);
static int close(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static void set_sensor_cali(void *arg);
static int luxl5uwide_set_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int luxl5uwide_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int luxl5uwide_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int luxl5uwide_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int luxl5uwide_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int luxl5uwide_get_otp_qcom_pdaf_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static unsigned int read_luxl5uwide_eeprom_4cell_info(struct subdrv_ctx *ctx,
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, luxl5uwide_set_test_pattern},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, luxl5uwide_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, luxl5uwide_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, luxl5uwide_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, luxl5uwide_streaming_resume},
	{SENSOR_FEATURE_SET_ESHUTTER, luxl5uwide_set_shutter},
	{SENSOR_FEATURE_SET_FRAMELENGTH, luxl5uwide_set_frame_length},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, luxl5uwide_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_GAIN, luxl5uwide_set_gain},
	{SENSOR_FEATURE_SET_HDR_SHUTTER, luxl5uwide_set_hdr_tri_shutter2},
	{SENSOR_FEATURE_SET_HDR_TRI_SHUTTER, luxl5uwide_set_hdr_tri_shutter3},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, luxl5uwide_set_multi_shutter_frame_length_ctrl},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_DATA, luxl5uwide_get_otp_qcom_pdaf_data},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, gc08a8_set_max_framerate_by_scenario},
	{SENSOR_FEATURE_GET_4CELL_DATA, luxl5uwide_get_eeprom_4cell_info},
};

static struct eeprom_info_struct eeprom_info[] = {
	{
		.header_id = 0x016F0131,
...
static struct subdrv_mode_struct mode_struct[] = {
	{
		...
		.mode_setting_table = luxl5uwide_preview_setting,
		.mode_setting_len = ARRAY_SIZE(luxl5uwide_preview_setting),
	...
	},
	{
		...
		.mode_setting_table = luxl5uwide_capture_setting,
		.mode_setting_len = ARRAY_SIZE(luxl5uwide_capture_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = luxl5uwide_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(luxl5uwide_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = luxl5uwide_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(luxl5uwide_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = luxl5uwide_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(luxl5uwide_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{ // GC08A8_2L_2368x1776_24fps_V1.0.txt
		.frame_desc = frame_desc_cus1,
		.num_entries = ARRAY_SIZE(frame_desc_cus1),
		.mode_setting_table = luxl5uwide_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(luxl5uwide_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
		...
		.framelength = 3205,
		.max_framerate = 240,
		.mipi_pixel_rate = 134400000, //672*2/10,
		.readout_length = 0,
		.read_margin = 0,
		.framelength_step = 1,
		.coarse_integ_step = 1,
	...
	{ // GC08A8_2L_1632x816_30fps_V1.0.txt
		.frame_desc = frame_desc_cus2,
		.num_entries = ARRAY_SIZE(frame_desc_cus2),
		.mode_setting_table = luxl5uwide_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(luxl5uwide_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = LUXL5UWIDE_SENSOR_ID,
	...
	.ana_gain_step = 1,
	.ana_gain_table = luxl5uwide_ana_gain_table,
	.ana_gain_table_size = sizeof(luxl5uwide_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	...
	.init_setting_table = luxl5uwide_sensor_init_setting,
	.init_setting_len =  ARRAY_SIZE(luxl5uwide_sensor_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry luxl5uwide_mipi_raw_entry = {
	.name = "luxl5uwide_mipi_raw",
	.id = LUXL5UWIDE_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void luxl5uwide_set_dummy(struct subdrv_ctx *ctx)
{
...
static void luxl5uwide_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate,
...
static int luxl5uwide_set_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	luxl5uwide_write_frame_length(ctx, ctx->frame_length);
...
static int luxl5uwide_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (set_auto_flicker(ctx, 0) || ctx->frame_length) {
		luxl5uwide_set_max_framerate(ctx, realtime_fps, 0);
	}
...
static void luxl5uwide_write_shutter(struct subdrv_ctx *ctx)
{
	kal_uint16 realtime_fps = 0;
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps >= 297 && realtime_fps <= 305) {
			luxl5uwide_set_max_framerate(ctx, 296, 0);
		} else if (realtime_fps >= 147 && realtime_fps <= 150) {
			luxl5uwide_set_max_framerate(ctx, 146, 0);
		} else {
			// Extend frame length
			subdrv_i2c_wr_u16(ctx, 0x0340, ctx->frame_length & 0xfffe);
		}
...
static void luxl5uwide_set_shutter_convert(struct subdrv_ctx *ctx, u32 *shutter)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *shutter);
	ctx->exposure[0] = *shutter;

	luxl5uwide_write_shutter(ctx);
}

static int luxl5uwide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *para);
	luxl5uwide_set_shutter_frame_length(ctx, para, len);
	return 0;
}
...
static int luxl5uwide_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
		...
		if (*(u32 *)para)
			luxl5uwide_set_shutter_convert(ctx, (u32 *)para);
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, LUXL5UWIDE_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id)
{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			printk("thdebug i2c_write_id 0x%x\n",ctx->i2c_write_id);
			*sensor_id = (subdrv_i2c_rd_u8(ctx, addr_h) << 8) |
			...
			if (*sensor_id == 0x08A8) {
				*sensor_id = ctx->s_ctx.sensor_id;
				if (first_read) {
					read_luxl5uwide_eeprom_4cell_info(ctx, &oplus_eeprom_4cell_info, oplus_eeprom_4cell_addr_table);
...
static void luxl5uwide_sensor_init(struct subdrv_ctx *ctx)
{
	for(int i = 0; i < ctx->s_ctx.init_setting_len; i += 2){
		subdrv_i2c_wr_u8(ctx, ctx->s_ctx.init_setting_table[i], ctx->s_ctx.init_setting_table[i+1]);
	}
}
...
static void luxl5uwide_write_custom1_setting(struct subdrv_ctx *ctx) {
	...
	subdrv_i2c_wr_u8(ctx, 0x0341, 0x85);
	subdrv_i2c_wr_u8(ctx, 0x0342, 0x07);
	subdrv_i2c_wr_u8(ctx, 0x0343, 0x1c);
	subdrv_i2c_wr_u8(ctx, 0x0226, 0x00);
	subdrv_i2c_wr_u8(ctx, 0x0227, 0x28);
	...
	subdrv_i2c_wr_u8(ctx, 0x0051, 0x44);
	subdrv_i2c_wr_u8(ctx, 0x009a, 0x00);
	subdrv_i2c_wr_u8(ctx, 0x0351, 0x01);
	subdrv_i2c_wr_u8(ctx, 0x0352, 0x56);
	subdrv_i2c_wr_u8(ctx, 0x0353, 0x01);
	subdrv_i2c_wr_u8(ctx, 0x0354, 0xc8);
	subdrv_i2c_wr_u8(ctx, 0x034c, 0x09);
	subdrv_i2c_wr_u8(ctx, 0x034d, 0x40);
	subdrv_i2c_wr_u8(ctx, 0x034e, 0x06);
	subdrv_i2c_wr_u8(ctx, 0x034f, 0xf0);
	subdrv_i2c_wr_u8(ctx, 0x0114, 0x01);
	subdrv_i2c_wr_u8(ctx, 0x0180, 0x67);
	subdrv_i2c_wr_u8(ctx, 0x0181, 0x30);
	subdrv_i2c_wr_u8(ctx, 0x0185, 0x01);
...
static void luxl5uwide_write_custom2_setting(struct subdrv_ctx *ctx) {
	...
	subdrv_i2c_wr_u8(ctx, 0x0342, 0x07);
	subdrv_i2c_wr_u8(ctx, 0x0343, 0x1c);
	subdrv_i2c_wr_u8(ctx, 0x0226, 0x04);
	subdrv_i2c_wr_u8(ctx, 0x0227, 0xfe);
	subdrv_i2c_wr_u8(ctx, 0x0e38, 0x39);
	subdrv_i2c_wr_u8(ctx, 0x0210, 0x53);
	subdrv_i2c_wr_u8(ctx, 0x0218, 0x80);
	subdrv_i2c_wr_u8(ctx, 0x0241, 0x8c);
	...
	subdrv_i2c_wr_u8(ctx, 0x009a, 0x00);
	subdrv_i2c_wr_u8(ctx, 0x0351, 0x00);
	subdrv_i2c_wr_u8(ctx, 0x0352, 0xd0);
	subdrv_i2c_wr_u8(ctx, 0x0353, 0x00);
	subdrv_i2c_wr_u8(ctx, 0x0354, 0x04);
	subdrv_i2c_wr_u8(ctx, 0x034c, 0x06);
	subdrv_i2c_wr_u8(ctx, 0x034d, 0x60);
	subdrv_i2c_wr_u8(ctx, 0x034e, 0x03);
	subdrv_i2c_wr_u8(ctx, 0x034f, 0x30);
	subdrv_i2c_wr_u8(ctx, 0x0114, 0x01);
	subdrv_i2c_wr_u8(ctx, 0x0180, 0x67);
	subdrv_i2c_wr_u8(ctx, 0x0181, 0x30);
	subdrv_i2c_wr_u8(ctx, 0x0185, 0x01);
...
static void luxl5uwide_write_discontinuity_mode(struct subdrv_ctx *ctx, enum SENSOR_SCENARIO_ID_ENUM scenario_id)
{
	for(int i = 0; i < ctx->s_ctx.mode[scenario_id].mode_setting_len; i += 2){
		subdrv_i2c_wr_u8(ctx, ctx->s_ctx.mode[scenario_id].mode_setting_table[i], ctx->s_ctx.mode[scenario_id].mode_setting_table[i+1]);
	}
	DRV_LOG(ctx, "mode %d\n", scenario_id);
}

static int luxl5uwide_control(struct subdrv_ctx *ctx,
		...
		if (ctx->s_ctx.aov_sensor_support &&
			...
			switch(scenario_id) {
				...
				case SENSOR_SCENARIO_ID_NORMAL_CAPTURE:
					luxl5uwide_write_preview_setting(ctx);
					break;
				case SENSOR_SCENARIO_ID_NORMAL_VIDEO:
					luxl5uwide_write_normal_video_setting(ctx);
					break;
				case SENSOR_SCENARIO_ID_HIGHSPEED_VIDEO:
				case SENSOR_SCENARIO_ID_SLIM_VIDEO:
					luxl5uwide_write_hs_video_setting(ctx);
					break;
				default:
					break;
				}
			...
			if(scenario_id <= ctx->s_ctx.sensor_mode_num)
				luxl5uwide_write_discontinuity_mode(ctx, scenario_id);
			else
				DRV_LOGE(ctx, "please check sensormode %d\n", scenario_id);
		}
...
static int open(struct subdrv_ctx *ctx)
{
	...
	if (get_imgsensor_id(ctx, &sensor_id) != ERROR_NONE)
		return ERROR_SENSOR_CONNECT_FAIL;
//	i2c_table_write(ctx, luxl5uwide_sensor_preinit_setting, ARRAY_SIZE(luxl5uwide_sensor_preinit_setting));
//	mdelay(5);
	luxl5uwide_sensor_init(ctx);
...
static void luxl5uwide_set_multi_shutter_frame_length(struct subdrv_ctx *ctx,
...
static int luxl5uwide_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	luxl5uwide_set_multi_shutter_frame_length(ctx, (u32 *)(*feature_data),
		(u16) (*(feature_data + 1)), (u16) (*(feature_data + 2)));
	return 0;
}

static void luxl5uwide_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt)
{
	int i = 0;
	u32 values[3] = {0};
	...
	luxl5uwide_set_multi_shutter_frame_length(ctx, values, exp_cnt, 0);
}
...
static int luxl5uwide_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	luxl5uwide_set_hdr_tri_shutter(ctx, feature_data, 2);
	return 0;
}
...
static int luxl5uwide_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	luxl5uwide_set_hdr_tri_shutter(ctx, feature_data, 3);
	return 0;
}
static void luxl5uwide_write_frame_length(struct subdrv_ctx *ctx, u32 fll)
{
