 ...
 *============================================================================
 ****************************************************************************/
#include "subarumainmipiraw_Sensor.h"
#define SENSOR_NAME  SENSOR_DRVNAME_SUBARUMAIN_MIPI_RAW
#define SUBARUMAIN_EEPROM_READ_ID	0xA1
#define SUBARUMAIN_EEPROM_WRITE_ID	0xA0
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "subarumain_camera_sensor"
...
#define SUBARUMAIN_STEREO_START_ADDR   (0x2FBA)
#define SUBARUMAIN_AESYNC_START_ADDR   (0x3CF0)

#define QSC_TYPE               (0x01)
#define LRC_TYPE               (0x02)
#define SUBARUMAIN_UNIQUE_SENSOR_ID_ADDR 0x3AE8
#define SUBARUMAIN_UNIQUE_SENSOR_ID_LENGTH 13

static BYTE subarumain_unique_id[SUBARUMAIN_UNIQUE_SENSOR_ID_LENGTH] = { 0 };
...
static int subarumain_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int subarumain_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int subarumain_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int subarumain_get_sensor_sn(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int subarumain_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int subarumain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int subarumain_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int subarumain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt);
static int subarumain_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static int subarumain_get_unique_sensorid(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int subarumain_set_cali_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int subarumain_get_cloud_otp_info(struct subdrv_ctx *ctx, u8 *para, u32 *len);
/* STRUCT */

static BYTE subarumain_common_data[OPLUS_CAMERA_COMMON_DATA_LENGTH] = { 0 };
...
static struct comp_mode_tran_time_params subarumain_comp_params[SENSOR_SCENARIO_ID_MAX] = {
	{ .enable = 0, }, /*pre*/
...
static struct eeprom_map_info subarumain_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, subarumain_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, subarumain_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, subarumain_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_DATA, subarumain_get_sensor_sn},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, subarumain_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, subarumain_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, subarumain_get_otp_checksum_data},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, subarumain_get_min_shutter_by_scenario_adapter},
	{SENSOR_FEATURE_SET_AWB_GAIN, subarumain_set_awb_gain},
	{SENSOR_FEATURE_GET_UNIQUE_SENSORID, subarumain_get_unique_sensorid},
	{SENSOR_FEATURE_SET_CALI_DATA, subarumain_set_cali_data},
	{SENSOR_FEATURE_GET_CLOUD_OTP_INFO, subarumain_get_cloud_otp_info},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{/*Reg_A_QBIN(VBIN)_4096x3072_30FPS with PDAF VB_max*/
		...
		.mode_setting_table = subarumain_preview_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_preview_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = subarumain_seamless_normal_preview,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(subarumain_seamless_normal_preview),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 7500,
		.framelength = 3900,
	...
	{/*Reg_A_QBIN(VBIN)_4096x3072_30FPS with PDAF VB_max*/
		...
		.mode_setting_table = subarumain_capture_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_capture_setting),
	...
	{/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = subarumain_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_normal_video_setting),
		.seamless_switch_group = 3,
		.seamless_switch_mode_setting_table = subarumain_seamless_normal_video,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(subarumain_seamless_normal_video),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 7500,
		.framelength = 3900,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = subarumain_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = subarumain_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = subarumain_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_A-1_QBIN(VBIN)_4096x3072_24FPS with PDAF VB_max**/
		...
		.mode_setting_table = subarumain_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_custom2_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = subarumain_seamless_custom2,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(subarumain_seamless_custom2),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 7500,
		.framelength = 4876,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = subarumain_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*F9-S4_4096x3072_30FPS_IZOOM+QBC**/
		...
		.mode_setting_table = subarumain_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_custom4_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = subarumain_seamless_custom4,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(subarumain_seamless_custom4),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 8960,
		.framelength = 3252,
    ...
    {/*F10-S1_4096x3072_24FPS_IZOOM+QBC**/
		...
		.mode_setting_table = subarumain_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_custom5_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = subarumain_seamless_custom5,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(subarumain_seamless_custom5),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 8960,
		.framelength = 4064,
    ...
    {/*Reg_F12-S6_4096x2304_30FPS**/
		...
		.mode_setting_table = subarumain_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_custom6_setting),
		.seamless_switch_group = 3,
		.seamless_switch_mode_setting_table = subarumain_seamless_custom6,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(subarumain_seamless_custom6),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 8960,
		.framelength = 3252,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = subarumain_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_custom7_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = subarumain_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_custom8_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = subarumain_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_custom9_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = subarumain_custom10_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_custom10_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {/*Reg_B_4096x2304_30FPS**/
		...
		.mode_setting_table = subarumain_custom11_setting,
		.mode_setting_len = ARRAY_SIZE(subarumain_custom11_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = SUBARUMAIN_SENSOR_ID,
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = subarumain_ana_gain_table,
	.ana_gain_table_size = sizeof(subarumain_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 6,
	...
	.init_setting_table = subarumain_init_setting,
	.init_setting_len = ARRAY_SIZE(subarumain_init_setting),
	.mode = mode_struct,
...
struct subdrv_entry subarumain_mipi_raw_entry = {
	.name = SENSOR_NAME,
	.id = SUBARUMAIN_SENSOR_ID,
	.pw_seq = pw_seq,
...
static unsigned int read_subarumain_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != subarumain_eeprom_info[meta_id].meta)
		return -1;

	if (size != subarumain_eeprom_info[meta_id].size)
		return -1;

	addr = subarumain_eeprom_info[meta_id].start;
	readsize = subarumain_eeprom_info[meta_id].size;
...
static int subarumain_get_sensor_sn(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	memcpy(feature_return_para_32, subarumain_common_data,
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, SUBARUMAIN_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, SUBARUMAIN_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, SUBARUMAIN_EEPROM_READ_ID >> 1, reg, (SUBARUMAIN_EEPROM_WRITE_ID & 0xFE) | 0x01);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, SUBARUMAIN_EEPROM_READ_ID >> 1, reg, SUBARUMAIN_EEPROM_WRITE_ID & 0xFE);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == SUBARUMAIN_SENSOR_ID || pStereodata->uSensorId == MILKYWAYS2MAIN_SENSOR_ID)
            && (data_length == CALI_DATA_SLAVE_LENGTH) && (data_base == SUBARUMAIN_STEREO_START_ADDR )) {
            ...
            */
        } else if ((pStereodata->uSensorId == SUBARUMAIN_SENSOR_ID || pStereodata->uSensorId == MILKYWAYS2MAIN_SENSOR_ID)
            && (data_length < AESYNC_DATA_LENGTH_TOTAL) && (data_base == SUBARUMAIN_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                read_cmos_eeprom_8(ctx, SUBARUMAIN_AESYNC_START_ADDR),
                read_cmos_eeprom_8(ctx, SUBARUMAIN_AESYNC_START_ADDR+1),
                read_cmos_eeprom_8(ctx, SUBARUMAIN_AESYNC_START_ADDR+2),
                read_cmos_eeprom_8(ctx, SUBARUMAIN_AESYNC_START_ADDR+3),
                read_cmos_eeprom_8(ctx, SUBARUMAIN_AESYNC_START_ADDR+4),
                read_cmos_eeprom_8(ctx, SUBARUMAIN_AESYNC_START_ADDR+5),
                read_cmos_eeprom_8(ctx, SUBARUMAIN_AESYNC_START_ADDR+6),
                read_cmos_eeprom_8(ctx, SUBARUMAIN_AESYNC_START_ADDR+7));
        ...
        LOG_INF("subarumain write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int subarumain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
    ...
    read_subarumain_eeprom_info(ctx, EEPROM_META_STEREO_DATA,
            (BYTE *)para, *len);
	return 0;
}
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, SUBARUMAIN_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "subarumain read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "subarumain read_otp_info end\n");
}
...
{
	u32 *feature_return_para_32 = (u32 *)para;
	*len = SUBARUMAIN_UNIQUE_SENSOR_ID_LENGTH;
	memcpy(feature_return_para_32, subarumain_unique_id,
		SUBARUMAIN_UNIQUE_SENSOR_ID_LENGTH);
...
static void read_unique_sensorid(struct subdrv_ctx *ctx)
{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		if (adaptor_i2c_rd_p8(ctx->i2c_client, ctx->i2c_write_id >> 1, SUBARUMAIN_UNIQUE_SENSOR_ID_ADDR,
			&(subarumain_unique_id[0]), SUBARUMAIN_UNIQUE_SENSOR_ID_LENGTH) < 0) {
...
static u16 subarumain_feedback_awbgain[] = {
	0x0B8E, 0x01,
	0x0B8F, 0x00,
	0x0B90, 0x02,
	0x0B91, 0x28,
...
static void feedback_awbgain(struct subdrv_ctx *ctx, kal_uint32 r_gain, kal_uint32 b_gain)
{
	...
	if (ctx->current_scenario_id == SENSOR_SCENARIO_ID_CUSTOM3 || // RMSC
		...
		subarumain_feedback_awbgain[5] = r_gain_int;
		subarumain_feedback_awbgain[7] = (r_gain - r_gain_int * 512) / 2;
		subarumain_feedback_awbgain[9] = b_gain_int;
		subarumain_feedback_awbgain[11] = (b_gain - b_gain_int * 512) / 2;
		subdrv_i2c_wr_regs_u8(ctx, subarumain_feedback_awbgain,
			ARRAY_SIZE(subarumain_feedback_awbgain));
	}
}
...
void subarumain_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
...
int subarumain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64 *feature_data = (u64 *) para;
	subarumain_get_min_shutter_by_scenario(ctx,
...
static void comp_mode_tran_time_cal1(struct subdrv_ctx *ctx, u32 scenario_id, u32* prsh) {
	...
	*prsh = 0U;
	if (subarumain_comp_params[ctx->current_scenario_id].clock_vtpxck == 0) {
	...
	system_delay = SYSTEM_DELAY1 * 1000 * 1000 * 10 /
		subarumain_comp_params[ctx->current_scenario_id].clock_vtpxck;
...
static void get_imgsensor_id_from_dts(struct subdrv_ctx *ctx, u32 *sensor_id) {
	struct subdrv_entry *m_subdrv_entry = &subarumain_mipi_raw_entry;
