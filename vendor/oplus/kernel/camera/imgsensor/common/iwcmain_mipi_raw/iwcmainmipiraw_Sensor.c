 ...
 *============================================================================
 ****************************************************************************/
#include "iwcmainmipiraw_Sensor.h"

#define SENSOR_NAME  SENSOR_DRVNAME_IWCMAIN_MIPI_RAW

#define PFX "iwcmain_camera_sensor"
...
#define IWCMAIN_STEREO_MW_START_ADDR     (0x4010)
#define IWCMAIN_STEREO_MT_START_ADDR     (0x4800)
#define IWCMAIN_STEREO_MT105_START_ADDR     (0x5000)
// #define IWCMAIN_AESYNC_START_ADDR     (0x574b)

// #define  OTP_XTC_ADDR             (0x1A00)
// #define  OTP_XTC_LENGTH           (9216)
// #define  OTP_XTC_VALID_ADDR       (0x3e00)
...
#define IWCMAIN_EEPROM_ADDR   (0xA0)
...
static int iwcmain_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int iwcmain_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int iwcmain_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int iwcmain_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int iwcmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int iwcmain_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int get_sensor_temperature(void *arg);
static void iwcmain_get_sensor_cali(void* arg);
static void iwcmain_set_sensor_cali(void *arg);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int iwcmain_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int iwcmain_i2c_burst_wr_regs_u16(struct subdrv_ctx *ctx, u16 * list, u32 len);
...
//////gain
static void iwcmain_set_gain_convert(struct subdrv_ctx *ctx, u32 gain);
static int iwcmain_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void iwcmain_set_multi_gain(struct subdrv_ctx *ctx, u32 *gains, u16 exp_cnt);
static void iwcmain_set_hdr_tri_gain(struct subdrv_ctx *ctx, u64 *gains, u16 exp_cnt);
static int iwcmain_set_hdr_tri_gain2(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int iwcmain_set_hdr_tri_gain3(struct subdrv_ctx *ctx, u8 *para, u32 *len);


///////shutter
static void iwcmain_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter);
static int iwcmain_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int iwcmain_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void iwcmain_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length);
static void iwcmain_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt, u16 frame_length);
static int iwcmain_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int iwcmain_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int iwcmain_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int iwcmain_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void iwcmain_set_multi_shutter_frame_length_in_lut_convert(struct subdrv_ctx *ctx,
	u64 *shutters, u16 exp_cnt, u32 frame_length, u32 *frame_length_in_lut);
static int iwcmain_set_multi_shutter_frame_length_in_lut(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void iwcmain_write_frame_length_in_lut(struct subdrv_ctx *ctx, u32 fll, u32 *fll_in_lut);
static void iwcmain_set_max_framerate_in_lut_by_scenario(struct subdrv_ctx *ctx,
	enum SENSOR_SCENARIO_ID_ENUM scenario_id, u32 framerate);
//static int iwcmain_chk_streaming_st(void *arg);
//static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt, u64 sof_ts);
/* STRUCT */

static struct eeprom_map_info iwcmain_eeprom_info[] = {
	...
	{ EEPROM_META_STEREO_MW_MAIN_DATA, IWCMAIN_STEREO_MW_START_ADDR, 0xFFFF, 0xFFFF, CALI_DATA_MASTER_LENGTH, true },
	{ EEPROM_META_STEREO_MT_MAIN_DATA, IWCMAIN_STEREO_MT_START_ADDR, 0xFFFF, 0xFFFF, CALI_DATA_MASTER_LENGTH, true },
	{ EEPROM_META_STEREO_MT_MAIN_DATA_105CM, IWCMAIN_STEREO_MT105_START_ADDR, 0xFFFF, 0xFFFF, CALI_DATA_MASTER_LENGTH, true },
};

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, iwcmain_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, iwcmain_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, iwcmain_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, iwcmain_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, iwcmain_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, iwcmain_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_AWB_GAIN, iwcmain_set_awb_gain},


	{SENSOR_FEATURE_SET_ESHUTTER, iwcmain_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, iwcmain_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_HDR_SHUTTER, iwcmain_set_hdr_tri_shutter2},
	{SENSOR_FEATURE_SET_HDR_TRI_SHUTTER, iwcmain_set_hdr_tri_shutter3},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, iwcmain_set_multi_shutter_frame_length_ctrl},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME_IN_LUT, iwcmain_set_multi_shutter_frame_length_in_lut},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, iwcmain_set_max_framerate_by_scenario},

	{SENSOR_FEATURE_SET_GAIN, iwcmain_set_gain},
	{SENSOR_FEATURE_SET_DUAL_GAIN, iwcmain_set_hdr_tri_gain2},
	{SENSOR_FEATURE_SET_HDR_TRI_GAIN, iwcmain_set_hdr_tri_gain3},
};

static struct eeprom_info_struct eeprom_info[] = {
	{
		.header_id = 0x015601B5,
		.addr_header_id = 0x00000008,
		.i2c_write_id = IWCMAIN_EEPROM_ADDR,
		// XTC/QPDC
		// .pdc_support = TRUE,
...
//static u32 iwcmain_dcg_ratio_table_10bit[] = {8000};
static u32 iwcmain_dcg_ratio_table_10bit_2[] = {4000};
static u32 iwcmain_dcg_ratio_table_12bit[] = {8000};
//static u32 iwcmain_dcg_ratio_table_14bit[] = {8000};
...
#define pd_i4Crop { \
	...
	{0, 0}, {1184, 888}, {2048, 1536}, {2048, 1536}, {2048, 1536},\
	/* cust11     cust12    cust13    cust14  cust15 */\
	{2048, 1920}, {0, 512}, {0, 384}, {0, 0}, {2048, 1536},\
...
/*0 8sum pre 4096x3072_30fps bring up*/
...
/*1 8sum cap 4096x3072_30fps bring up*/
...
/*2 8sum Normal_video 4096x2304_30fps bring up*/
...
/*3 8sum Normal_hs_video 4096x2304_60fps bring up*/
...
/*4 8sum 120fps_slim_video 2048x1152_120fps bring up */
...
/*5 8sum 240fps_slim_video 2048x1152_240fps bring up */
...
/*6 8sum portrait 4096x3072_24fps bring up */
...
/*7 full 200M 16384x12288_7.5fps bring up */
...
/*8 4sum 50M 8192x6144_30fps sensor_remosaic on bring up */
...
/*9 8sum video 4096x3072_30fps bring up */
...
/*10 8sum hs_video 4096x3072_60fps bring up */
...
/*11 4sum Normal_2x_qbc 4096x3072_30fps bring up*/
...
/*12 4sum Normal_2x_izoom 4096x3072_30fps bring up */
...
/*13 4sum portrait_2x_qbc 4096x3072_24fps bring up*/
...
/*14 4sum portrait_2x_izoom 4096x3072_24fps bring up */
...
/*15 full Normal_4x_qbc 4096x3072_30fps bring up*/
...
/*16 full portrait_4x_qbc 4096x3072_24fps bring up*/
...
/*17 8sum iDCG 4096x2304_30fps 12bit bring up */
...
/*18 8sum iDCG 4096x2304_24fps 12bit bring up */
...
/*19 8sum small_size 2048x1536_24fps bring up */
...
/*20 8sum small_size 2048x1536_30fps bring up */
...
/*21 8sum small_size 1728x1296_24fps bring up */
...
/*22 8sum small_size 3840x2160_60fps bring up*/
...
static struct subdrv_mode_struct mode_struct[] = {
	{/*0 8sum pre 4096x3072_30fps bring up*/
		...
		.mode_setting_table = iwcmain_preview_capture_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_preview_capture_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = iwcmain_preview_capture_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(iwcmain_preview_capture_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{/*2 8sum Normal_video 4096x2304_30fps bring up*/
		...
		.mode_setting_table = iwcmain_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*3 8sum Normal_hs_video 4096x2304_60fps bring up*/
		...
		.mode_setting_table = iwcmain_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*4 8sum 120fps_slim_video 2048x1152_120fps bring up */
		...
		.mode_setting_table = iwcmain_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
		...
		.imgsensor_winsize_info = {
			.full_w = 16384,
			.full_h = 12288,
			.x0_offset = (16384-2048*4)/2,
			.y0_offset = (12288-1152*4)/2,
			.w0_size = 2048*4,
			.h0_size = 1152*4,
			.scale_w = 2048,
			.scale_h = 1152,
	...
	{/*5 8sum 240fps_slim_video 2048x1152_240fps bring up */
		...
		.mode_setting_table = iwcmain_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*6 8sum portrait 4096x3072_24fps bring up */
		...
		.mode_setting_table = iwcmain_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom2_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = iwcmain_custom2_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(iwcmain_custom2_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{ /*7 full 200M 16384x12288_7.5fps bring up */
		...
		.mode_setting_table = iwcmain_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom3_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = iwcmain_custom3_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(iwcmain_custom3_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{/*8 4sum 50M 8192x6144_30fps sensor_remosaic on bring up */
		...
		.mode_setting_table = iwcmain_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom4_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = iwcmain_custom4_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(iwcmain_custom4_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{ /*9 8sum video 4096x3072_30fps bring up */
		...
		.mode_setting_table = iwcmain_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom5_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{ /*10 8sum hs_video 4096x3072_60fps bring up */
		...
		.mode_setting_table = iwcmain_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom6_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{  /*11 4sum Normal_2x_qbc 4096x3072_30fps bring up*/
		...
		.mode_setting_table = iwcmain_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom7_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = iwcmain_custom7_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(iwcmain_custom7_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{ /*12 4sum Normal_2x_izoom 4096x3072_30fps bring up */
		...
		.mode_setting_table = iwcmain_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom8_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = iwcmain_custom8_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(iwcmain_custom8_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{/*13 4sum portrait_2x_qbc 4096x3072_24fps bring up*/
		...
		.mode_setting_table = iwcmain_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom9_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*14 4sum portrait_2x_izoom 4096x3072_24fps bring up */
		...
		.mode_setting_table = iwcmain_custom10_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom10_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = iwcmain_custom10_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(iwcmain_custom10_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{/*15 full Normal_4x_qbc 4096x3072_30fps bring up*/
		...
		.mode_setting_table = iwcmain_custom11_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom11_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = iwcmain_custom11_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(iwcmain_custom11_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{/*16 full portrait_4x_qbc 4096x3072_24fps bring up*/
		...
		.mode_setting_table = iwcmain_custom12_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom12_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*17 8sum iDCG 4096x2304_30fps 12bit bring up */
		...
		.mode_setting_table = iwcmain_custom13_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom13_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		...
		.dcg_info = {
			...
			.dcg_gain_table = iwcmain_dcg_ratio_table_12bit,
			.dcg_gain_table_size = sizeof(iwcmain_dcg_ratio_table_12bit),
		},
		.multi_exposure_shutter_range[IMGSENSOR_EXPOSURE_LE].min = 4,
	...
	{/*18  8sum iDCG 4096x2304_24fps 12bit bring up */
		...
		.mode_setting_table = iwcmain_custom14_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom14_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_RAW_DCG_COMPOSE,
	...
	{/*19 8sum small_size 2048x1536_24fps bring up */
		...
		.mode_setting_table = iwcmain_custom15_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom15_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*20 8sum small_size 2048x1536_30fps bring up */
		...
		.mode_setting_table = iwcmain_custom16_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom16_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*21 8sum small_size 1728x1296_24fps bring up */
		...
		.mode_setting_table = iwcmain_custom17_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom17_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
		...
		.imgsensor_winsize_info = {
			.full_w = 16384,
			.full_h = 12288,
			.x0_offset = (16384-1728*4)/2,
			.y0_offset = (12288-1296*4)/2,
			.w0_size = 1728*4,
			.h0_size = 1296*4,
			.scale_w = 1728,
			.scale_h = 1296,
	...
	{/*22 8sum small_size 3840x2160_60fps bring up*/
		...
		.mode_setting_table = iwcmain_custom18_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom18_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*23 8sum iDCG 4096x3072_30fps 10bit bring up */
		.frame_desc = frame_desc_cus19,
		.num_entries = ARRAY_SIZE(frame_desc_cus19),
		.mode_setting_table = iwcmain_custom19_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom19_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_RAW_DCG_COMPOSE,
		...
		.dcg_info = {
			...
			.dcg_gain_ratio_step = 0,
			.dcg_gain_table = iwcmain_dcg_ratio_table_10bit_2,
			.dcg_gain_table_size = sizeof(iwcmain_dcg_ratio_table_10bit_2),
		},
		// .sensor_setting_info = {
	...
	{/*24 Al Snap Mode 8sum pre 4096x3072_30fps bring up*/
		.frame_desc = frame_desc_cus20,
		.num_entries = ARRAY_SIZE(frame_desc_cus20),
		.mode_setting_table = iwcmain_custom20_setting,
		.mode_setting_len = ARRAY_SIZE(iwcmain_custom20_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = iwcmain_custom20_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(iwcmain_custom20_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
...
static struct subdrv_static_ctx static_ctx = {
	...
	//Enhance mlck driving capability
	// .mclk = 24,
	...
	.ana_gain_step = 1,
	.ana_gain_table = iwcmain_ana_gain_table,
	.ana_gain_table_size = sizeof(iwcmain_ana_gain_table),
	//.tuning_iso_base = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	.exposure_max = (0xFFFF - 0x46) << 9,
	.exposure_step = 1,
	.exposure_margin = 0x28,
...
//	.stagger_fl_type = IMGSENSOR_STAGGER_FL_MANUAL,
	...
	.g_cali = iwcmain_get_sensor_cali,
	.s_gph = set_group_hold,
	.s_cali = iwcmain_set_sensor_cali,
...
//	.reg_addr_frame_length_lshift = 0x0702, /*frame_length_lines_sh*/
	...
	// .init_setting_table = iwcmain_init_setting,
	// .init_setting_len = ARRAY_SIZE(iwcmain_init_setting),
	.mode = mode_struct,
	...
	.chk_s_off_sta = 1,
	.chk_s_off_end = 0,
//	.chk_streaming_st = iwcmain_chk_streaming_st,
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	...
	// {HW_ID_AVDD, 2200000, 1000},
	// {HW_ID_DVDD, 1002000, 1000},
	...
	{HW_ID_DOVDD, 1800000, 1},
	{HW_ID_PDN, 0, 3},
	{HW_ID_AVDD, 2204000, 3},
	{HW_ID_AVDD1, 1804000, 3},
	{HW_ID_AFVDD, 2800000, 3},
	{HW_ID_DVDD, 1000000, 4},
	//{HW_ID_DOVDD, 1800000, 3},
	{HW_ID_RST, 1, 2},
	{HW_ID_MCLK_DRIVING_CURRENT, 4, 11},
};
struct subdrv_entry iwcmain_mipi_raw_entry = {

	.name = "iwcmain_mipi_raw",
	.id = IWCMAIN_SENSOR_ID, /* 0x1b73 */
	.pw_seq = pw_seq,
...
static unsigned int read_iwcmain_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != iwcmain_eeprom_info[meta_id].meta)
		return -1;

	if (size != iwcmain_eeprom_info[meta_id].size)
		return -1;

	addr = iwcmain_eeprom_info[meta_id].start;
	readsize = iwcmain_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
    ...
    adaptor_i2c_rd_u8(ctx->i2c_client, IWCMAIN_EEPROM_ADDR >> 1, addr, (u8 *)&get_byte);
    return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, IWCMAIN_EEPROM_ADDR >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, IWCMAIN_EEPROM_ADDR >> 1, reg, 0x0E);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, IWCMAIN_EEPROM_ADDR >> 1, reg, 0x00);
    }
...
//         if (((pStereodata->uSensorId == IWCMAIN_SENSOR_ID) || (pStereodata->uSensorId == CASIOY1MAIN2_SENSOR_ID)
//             || (pStereodata->uSensorId == CASIOY2MAIN2_SENSOR_ID) || (pStereodata->uSensorId == CASIOY3MAIN2_SENSOR_ID))
//             && (data_length == CALI_DATA_MASTER_LENGTH)
//             && ((data_base == IWCMAIN_STEREO_MW_START_ADDR)
//                 || (data_base == IWCMAIN_STEREO_MT_START_ADDR)
//                 || (data_base == IWCMAIN_STEREO_MT105_START_ADDR))) {
...
//         } else if (((pStereodata->uSensorId == IWCMAIN_SENSOR_ID) || (pStereodata->uSensorId == WHOOPASSS1MAIN_SENSOR_ID)
//             || (pStereodata->uSensorId == IWCMAIN_SENSOR_ID) || (pStereodata->uSensorId == WHOOPASSS2MAIN_SENSOR_ID))
//             && (data_length < AESYNC_DATA_LENGTH_TOTAL)
//             && (data_base == IWCMAIN_AESYNC_START_ADDR)) {
...
//                     read_cmos_eeprom_8(ctx, IWCMAIN_AESYNC_START_ADDR),
//                     read_cmos_eeprom_8(ctx, IWCMAIN_AESYNC_START_ADDR+1),
//                     read_cmos_eeprom_8(ctx, IWCMAIN_AESYNC_START_ADDR+2),
//                     read_cmos_eeprom_8(ctx, IWCMAIN_AESYNC_START_ADDR+3),
//                     read_cmos_eeprom_8(ctx, IWCMAIN_AESYNC_START_ADDR+4),
//                     read_cmos_eeprom_8(ctx, IWCMAIN_AESYNC_START_ADDR+5),
//                     read_cmos_eeprom_8(ctx, IWCMAIN_AESYNC_START_ADDR+6),
//                     read_cmos_eeprom_8(ctx, IWCMAIN_AESYNC_START_ADDR+7));
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        data_length = pStereodata->dataLength;
        pData = pStereodata->uData;
        if ((pStereodata->uSensorId == IWCMAIN_SENSOR_ID) && (data_length == CALI_DATA_MASTER_LENGTH)
            && (data_base == IWCMAIN_STEREO_MW_START_ADDR)) {
            LOG_INF("Write: %x %x %x %x\n", pData[0], pData[39], pData[40], pData[1556]);
        ...
        } else if ((pStereodata->uSensorId == IWCMAIN_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == IWCMAIN_AESYNC_START_ADDR)) {
            LOG_INF("write main aesync: %x %x %x %x %x %x %x %x\n", pData[0], pData[1],
                pData[2], pData[3], pData[4], pData[5], pData[6], pData[7]);
...
static int iwcmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	case EEPROM_STEREODATA_MT_MAIN:
		read_iwcmain_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA,
				(BYTE *)feature_return_para_32, *len);
		break;
	case EEPROM_STEREODATA_MW_MAIN:
		read_iwcmain_eeprom_info(ctx, EEPROM_META_STEREO_MW_MAIN_DATA,
				(BYTE *)feature_return_para_32, *len);
		break;
	case EEPROM_STEREODATA_MT_MAIN_105CM:
		read_iwcmain_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA_105CM,
				(BYTE *)feature_return_para_32, *len);
		break;
	default:
		break;
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, IWCMAIN_EEPROM_ADDR >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
static int iwcmain_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len) {
	...
	LOG_INF("ABS_GAIN_GR(%d) ABS_GAIN_R(%d) ABS_GAIN_B(%d) ABS_GAIN_GB(%d)", awb_gain->ABS_GAIN_GR, awb_gain->ABS_GAIN_R, awb_gain->ABS_GAIN_B, awb_gain->ABS_GAIN_GB);
//	LOG_INF("0x0D82(red) = (0x%x)", subdrv_i2c_rd_u16(ctx, 0x0D82));
//	LOG_INF("0x0D84(green) = (0x%x)", subdrv_i2c_rd_u16(ctx, 0x0D84));
//	LOG_INF("0x0D86(blue) = (0x%x)", subdrv_i2c_rd_u16(ctx, 0x0D86));
	return 0;
}
...
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id)
{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		LOG_INF("i2c_write_id(0x%x)\n",ctx->i2c_write_id);
		LOG_INF("ctx->s_ctx.i2c_addr_table[i](0x%x)\n",ctx->s_ctx.i2c_addr_table[i]);
		do {
...
static int open(struct subdrv_ctx *ctx)
{
	...
	DRV_LOGE(ctx, "module_flag = 0x%x\n", module_flag);

	if ((module_flag & 0xFF00) == 0x0200){
		subdrv_i2c_wr_regs_u16(ctx, iwcmain_init_setting_part1, ARRAY_SIZE(iwcmain_init_setting_part1));
		mdelay(1);
		subdrv_i2c_wr_regs_u16(ctx, iwcmain_init_setting_part2, ARRAY_SIZE(iwcmain_init_setting_part2));
		mdelay(1);
		iwcmain_i2c_burst_wr_regs_u16(ctx, iwcmain_init_setting_part3_094, ARRAY_SIZE(iwcmain_init_setting_part3_094));
		ctx->s_ctx.init_setting_len = ARRAY_SIZE(iwcmain_init_setting_part1)
											+ ARRAY_SIZE(iwcmain_init_setting_part2)
											+ ARRAY_SIZE(iwcmain_init_setting_part3_094);
	}else{
		subdrv_i2c_wr_regs_u16(ctx, iwcmain_init_setting_part1, ARRAY_SIZE(iwcmain_init_setting_part1));
		mdelay(1);
		subdrv_i2c_wr_regs_u16(ctx, iwcmain_init_setting_part2, ARRAY_SIZE(iwcmain_init_setting_part2));
		mdelay(1);
		iwcmain_i2c_burst_wr_regs_u16(ctx, iwcmain_init_setting_part3_092, ARRAY_SIZE(iwcmain_init_setting_part3_092));
		ctx->s_ctx.init_setting_len = ARRAY_SIZE(iwcmain_init_setting_part1)
											+ ARRAY_SIZE(iwcmain_init_setting_part2)
											+ ARRAY_SIZE(iwcmain_init_setting_part3_092);
	}
	...
	// 		ARRAY_SIZE(iwcmain_init_setting_part1)
	// 		+ ARRAY_SIZE(iwcmain_init_setting_part2)
	// 		+ ARRAY_SIZE(iwcmain_init_setting_part3);
	// }
...
static int iwcmain_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ae_ctrl) {
		...
		case HDR_RAW_STAGGER:
			iwcmain_set_multi_shutter_frame_length(ctx, (u64 *)&ae_ctrl->exposure, exp_cnt, 0);
			iwcmain_set_multi_gain(ctx, (u32 *)&ae_ctrl->gain, exp_cnt);
			break;
		case HDR_RAW_LBMF:
			iwcmain_set_multi_shutter_frame_length_in_lut_convert(ctx,
				(u64 *)&ae_ctrl->exposure, exp_cnt, 0, frame_length_in_lut);
			set_multi_gain_in_lut(ctx, (u32 *)&ae_ctrl->gain, exp_cnt);
			break;
		case HDR_RAW_DCG_RAW:
			iwcmain_set_multi_shutter_frame_length(ctx, (u64 *)&ae_ctrl->exposure, 1, 0);
			if (ctx->s_ctx.mode[scenario_id].dcg_info.dcg_gain_mode
				== IMGSENSOR_DCG_DIRECT_MODE)
				iwcmain_set_multi_gain(ctx, (u32 *)&ae_ctrl->gain, exp_cnt);
			else
				iwcmain_set_gain_convert(ctx, ae_ctrl->gain.me_gain);
			break;
		default:
			iwcmain_set_shutter_convert(ctx, ae_ctrl->exposure.le_exposure);
			iwcmain_set_gain_convert(ctx, ae_ctrl->gain.le_gain);
			break;
		}
	}
	commit_i2c_buffer(ctx);
...
void iwcmain_get_sensor_cali(void* arg)
{
	struct subdrv_ctx *ctx = (struct subdrv_ctx *)arg;
...
*/
static void get_imgsensor_id_from_dts(struct subdrv_ctx *ctx, u32 *sensor_id) {
	struct subdrv_entry *m_subdrv_entry = &iwcmain_mipi_raw_entry;
...
void iwcmain_write_frame_length(struct subdrv_ctx *ctx, u32 fll)
{
...
static void iwcmain_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt)
{
	int i = 0;
	u64 values[3] = {0};
	u32 frame_length_in_lut[IMGSENSOR_STAGGER_EXPOSURE_CNT] = {0};
	...
	iwcmain_set_multi_shutter_frame_length(ctx, values, exp_cnt, 0);
}
...
static int iwcmain_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	iwcmain_set_hdr_tri_shutter(ctx, feature_data, 2);
	return 0;
}
...
static int iwcmain_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	iwcmain_set_hdr_tri_shutter(ctx, feature_data, 3);
	return 0;
}


bool iwcmain_set_long_exposure(struct subdrv_ctx *ctx,  u64 shutter)
{
	u16 l_shift = 0;
	u16 l_shift_Max = 9;
...
LOG_INF("lzl_shutter:%llu, frame_length_max:%u  exposure_margin:%d\n", shutter, ctx->s_ctx.frame_length_max, exposure_margin);
	if (shutter > (ctx->s_ctx.frame_length_max - exposure_margin)) {
		...
		for (l_shift = 1; l_shift < l_shift_Max; l_shift++) {
			if ((shutter >> l_shift)
				< (ctx->s_ctx.frame_length_max - exposure_margin))
				break;
		}
		if (l_shift > l_shift_Max) {
...
static void iwcmain_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt, u16 frame_length)
{
	...
	if(exp_cnt == 1) {  //force to 1exp func
  		iwcmain_set_shutter_frame_length_convert(ctx, shutters[0], frame_length);
  		return ;
  	}else if (exp_cnt == 2) {
	...
	fll = frame_length ? frame_length : ctx->frame_length;
	...
	if (set_auto_flicker(ctx, 0) || frame_length || !ctx->s_ctx.reg_addr_auto_extend)
		iwcmain_write_frame_length(ctx, ctx->frame_length);
	else if (ctx->s_ctx.reg_addr_auto_extend)
		iwcmain_write_frame_length(ctx, ctx->min_frame_length);
	/* write shutter */
	switch (exp_cnt) {
	case 1:
...
static int iwcmain_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	iwcmain_set_multi_shutter_frame_length(ctx, (u64 *)(*feature_data),
...
static void iwcmain_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length)
{
	...
	if(iwcmain_set_long_exposure(ctx, shutter)) {
		goto exit;
	}
...
static int iwcmain_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
 	iwcmain_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], ((u64*)para)[1]);
	return 0;
}

static void iwcmain_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter)
{
    iwcmain_set_shutter_frame_length_convert(ctx, shutter, 0);
}

static int iwcmain_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	iwcmain_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], 0);
	return 0;
}
	...
	u64 *shutters, u16 exp_cnt, u32 frame_length, u32 *frame_length_in_lut)
{
	...
	iwcmain_write_frame_length_in_lut(ctx, ctx->frame_length, ctx->frame_length_in_lut);
...
void iwcmain_set_dummy(struct subdrv_ctx *ctx)
{
	bool gph = !ctx->is_seamless && (ctx->s_ctx.s_gph != NULL);
...
static int iwcmain_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ctx->s_ctx.mode[scenario_id].hdr_mode == HDR_RAW_LBMF) {
		iwcmain_set_max_framerate_in_lut_by_scenario(ctx, scenario_id, framerate);
		return 0;
	}
...
static int iwcmain_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	iwcmain_set_gain_convert(ctx, gain);
	return 0;
}
...
static void iwcmain_set_hdr_tri_gain(struct subdrv_ctx *ctx, u64 *gains, u16 exp_cnt)
{
	...
	iwcmain_set_multi_gain(ctx,	values, exp_cnt);
}

static int iwcmain_set_hdr_tri_gain2(struct subdrv_ctx *ctx, u8 *para, u32 *len) {

	u64 *feature_data = (u64 *) para;
	iwcmain_set_hdr_tri_gain(ctx, feature_data, 2);
	return 0;
}

static int iwcmain_set_hdr_tri_gain3(struct subdrv_ctx *ctx, u8 *para, u32 *len) {
	u64 *feature_data = (u64 *) para;
	iwcmain_set_hdr_tri_gain(ctx, feature_data, 3);
	return 0;
}
