 ...
 *============================================================================
 ****************************************************************************/
#include "cruiserhmain2mipiraw_Sensor.h"

#define SENSOR_NAME  SENSOR_DRVNAME_CRUISERHMAIN2_MIPI_RAW

#define CRUISERHMAIN2_EEPROM_ADDR        (0xA0)
#define CRUISERHMAIN2_EERPOM_MAX_OFFSET  (0x4000)
#define CRUISERHMAIN2_AESYNC_START_ADDR     (0x2F90)

#define PFX "cruiserhmain2_camera_sensor"
...
#define CRUISERHMAIN2_STEREO_MW_START_ADDR  (0x2980)
...
#define CRUISERHMAIN2_IMGSENSOR_ID   (0xb787)

#define CRUISERHMAIN2_UNIQUE_SENSOR_ID_ADDR    (0x7000)
#define CRUISERHMAIN2_UNIQUE_SENSOR_ID_LENGTH  (16)
/* seamless功能相关定义 */
...
static kal_uint8 otp_data_checksum[CRUISERHMAIN2_EERPOM_MAX_OFFSET] = {0};
static int get_sensor_temperature(void *arg);
#define MAX_BURST_LEN  (2048)
static u8 * msg_buf = NULL;
static void set_group_hold(void *arg, u8 en);

static void cruiserhmain2_set_mirror_flip(struct subdrv_ctx *ctx, u8 image_mirror);
static int cruiserhmain2_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain2_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain2_get_eeprom_comdata(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain2_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain2_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain2_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain2_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt);
static int cruiserhmain2_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain2_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void cruiserhmain2_set_max_framerate(struct subdrv_ctx *ctx, kal_uint16 framerate, kal_bool min_framelength_en);
static void cruiserhmain2_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length);
static void cruiserhmain2_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt, u16 frame_length);
static int cruiserhmain2_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain2_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain2_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int cruiserhmain2_i2c_burst_wr_regs_u8(struct subdrv_ctx *ctx, u16 * list, u32 len);
static int adapter_i2c_burst_wr_regs_u8(struct subdrv_ctx * ctx,
		u16 addr, u16 *list, u32 len);
static void cruiserhmain2_get_sensor_cali(void* arg);
static void cruiserhmain2_set_sensor_cali(void *arg);
static int cruiserhmain2_get_readout_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain2_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cruiserhmain2_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void streaming_ctrl(struct subdrv_ctx *ctx, bool enable);

/* 新增seamless相关函数声明 */
static void cruiserhmain2_seamless_start(struct subdrv_ctx *ctx);
static void cruiserhmain2_seamless_end(struct subdrv_ctx *ctx, kal_uint32 curr_vts, kal_uint32 curr_shutter, kal_uint32 tline_ns);
static kal_uint32 cruiserhmain2_get_tline_ns(struct subdrv_ctx *ctx, kal_uint32 curr_hts, kal_uint32 curr_pclk);
static int cruiserhmain2_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
// static int cruiserhmain2_get_seamless_scenarios(struct subdrv_ctx *ctx, u8 *para, u32 *len);

static void cruiserhmain2_feedback_awbgain(struct subdrv_ctx *ctx, kal_uint32 r_gain, kal_uint32 b_gain);
static int cruiserhmain2_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static u16 get_gain2reg(u32 gain);
static int cruiserhmain2_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);

// static void update_CTLE(struct subdrv_ctx *ctx);
// #define WIDEC1_CTLE_LEVEL 1
// #define WIDEC1_CTLE_EQBW 3

static struct eeprom_map_info cruiserhmain2_eeprom_info[] = {
	...
	{ EEPROM_META_STEREO_MW_MAIN_DATA, CRUISERHMAIN2_STEREO_MW_START_ADDR, 0xFFFF, 0xFFFF, CALI_DATA_SLAVE_LENGTH, false },
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, cruiserhmain2_set_test_pattern},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, cruiserhmain2_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, cruiserhmain2_get_eeprom_comdata},
	{SENSOR_FEATURE_SET_SENSOR_OTP, cruiserhmain2_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, cruiserhmain2_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, cruiserhmain2_get_otp_checksum_data},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, cruiserhmain2_get_min_shutter_by_scenario_adapter},
	{SENSOR_FEATURE_SET_ESHUTTER, cruiserhmain2_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, cruiserhmain2_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_HDR_SHUTTER, cruiserhmain2_set_hdr_tri_shutter2},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, cruiserhmain2_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, cruiserhmain2_streaming_resume},
	{SENSOR_FEATURE_SET_HDR_TRI_SHUTTER, cruiserhmain2_set_hdr_tri_shutter3},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, cruiserhmain2_set_multi_shutter_frame_length_ctrl},
	{SENSOR_FEATURE_SET_GAIN, cruiserhmain2_set_gain},
	{SENSOR_FEATURE_GET_READOUT_BY_SCENARIO, cruiserhmain2_get_readout_by_scenario},
	{SENSOR_FEATURE_SET_AWB_GAIN, cruiserhmain2_set_awb_gain},
	/* 新增seamless功能*/
	{SENSOR_FEATURE_SEAMLESS_SWITCH, cruiserhmain2_seamless_switch},
//	{SENSOR_FEATURE_GET_SEAMLESS_SCENARIOS, cruiserhmain2_get_seamless_scenarios},
};
...
static struct eeprom_info_struct eeprom_info[] = {
	{
		...
		.i2c_write_id = CRUISERHMAIN2_EEPROM_ADDR,
		.pdc_support = TRUE,
...
static u32 cruiserhmain2_dcg_ratio_table_12bit[] = {4000};

static u32 cruiserhmain2_dcg_ratio_table_10bit[] = {4000};
...
static struct subdrv_mode_struct mode_struct[] = {
    {//mode 0 G02_RSS_4096x3072_modeh_1536Msps_pd_type2_30.085470fps_cphy_3trio_10bit_valid_disc_20250922_V0.1.ini
		.frame_desc = frame_desc_prev_cap,
		.num_entries = ARRAY_SIZE(frame_desc_prev_cap),
		.mode_setting_table = cruiserhmain2_preview_capture_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_preview_capture_setting),
		/* 添加seamless配置 */
	...
	{//mode 1 G02_RSS_4096x3072_modeh_1536Msps_pd_type2_30.085470fps_cphy_3trio_10bit_valid_disc_20250922_V0.1.ini
		.frame_desc = frame_desc_prev_cap,
		.num_entries = ARRAY_SIZE(frame_desc_prev_cap),
		.mode_setting_table = cruiserhmain2_preview_capture_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_preview_capture_setting),
	...
	{// mode 2G06_RSS_Crop_4096x2304_modeh_1536Msps_pd_type2_30.085470fps_cphy_3trio_10bit_valid_disc_20250922_V0.1.ini
		.frame_desc = frame_desc_vid,
		.num_entries = ARRAY_SIZE(frame_desc_vid),
		.mode_setting_table = cruiserhmain2_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {// mode 3 G07_Summing_Crop_4096x2304_6%_1536Msps_pd_type2_60.053381fps_cphy_3trio_10bit_20250922_V0.1.ini
		.frame_desc = frame_desc_hs,
		.num_entries = ARRAY_SIZE(frame_desc_hs),
		.mode_setting_table = cruiserhmain2_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {// mode 4 G08_RSS_V2H2_1920x1080_1536Msps_120.325535fps_pd_type2_cphy_3trio_10bit_valid_disc_20250922_V0.1.ini
		.frame_desc = frame_desc_slim,
		.num_entries = ARRAY_SIZE(frame_desc_slim),
		.mode_setting_table = cruiserhmain2_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {// mode 5 G09_V2H2_1920x1080_1536Msps_240.590248fps_no_pd_cphy_3trio_10bit_20250922_V0.1.ini
		.frame_desc = frame_desc_cus1,
		.num_entries = ARRAY_SIZE(frame_desc_cus1),
		.mode_setting_table = cruiserhmain2_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
    ...
    {// mode 6 G04_RSS_4096x3072_modeh_1536Msps_pd_type2_24.004364fps_cphy_3trio_10bit_valid_disc_20250922_V0.1.ini
		.frame_desc = frame_desc_cus2,
		.num_entries = ARRAY_SIZE(frame_desc_cus2),
		.mode_setting_table = cruiserhmain2_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_custom2_setting),
    ...
    {// mode 7 G01_FullSize_8192x6144_15.009005fps_no_pd_1536Msps_bit10_CIP_on_QSC_on_20250922_V0.1.ini
		.frame_desc = frame_desc_cus3,
		.num_entries = ARRAY_SIZE(frame_desc_cus3),
		.mode_setting_table = cruiserhmain2_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_custom3_setting),
	...
	{// mode 8 G03_FullSize_crop_4096x3072_30.026690fps_pd_type2_1536Msps_bit10_CIP_on_QSC_on_20250922_V0.1.ini
		.frame_desc = frame_desc_cus4,
		.num_entries = ARRAY_SIZE(frame_desc_cus4),
		.mode_setting_table = cruiserhmain2_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_custom4_setting),
	...
	{// mode 9 G10_RSS_Crop_4096x2048_modeh_1536Msps_pd_type2_30.085470fps_cphy_3trio_10bit_valid_disc_20250922_V0.1.ini
		.frame_desc = frame_desc_cus5,
		.num_entries = ARRAY_SIZE(frame_desc_cus5),
		.mode_setting_table = cruiserhmain2_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_custom5_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 131625000,
	...
	{// mode 10 G05_FullSize_crop_4096x3072_24.021352fps_pd_type2_1536Msps_bit10_CIP_on_QSC_on_20250922_V0.1.ini
		.frame_desc = frame_desc_cus6,
		.num_entries = ARRAY_SIZE(frame_desc_cus6),
		.mode_setting_table = cruiserhmain2_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_custom6_setting),
	...
	{// mode 11 G12_RSS_Crop_PGHDR_4096x2304_WI_DCG_Combine_1536Msps_pdtype2_30.054644fps_cphy_trio3_12bit_20251105_V0.2.ini
		...
		.mode_setting_table = cruiserhmain2_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_custom7_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		...
		.dcg_info = {
			...
			.dcg_gain_table = cruiserhmain2_dcg_ratio_table_12bit,
			.dcg_gain_table_size = sizeof(cruiserhmain2_dcg_ratio_table_12bit),
		},
	...
	{// mode 12 G14_RSS_Crop_PGHDR_4096x2304_WI_DCG_Combine_1536Msps_pdtype2_30.054645fps_cphy_trio3_12bit_valid_disc_20251029.ini
		...
		.mode_setting_table = cruiserhmain2_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_custom8_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		...
		.dcg_info = {
			...
			.dcg_gain_ratio_max = 4000,
			.dcg_gain_ratio_step = 0,
			.dcg_gain_table = cruiserhmain2_dcg_ratio_table_10bit,
			.dcg_gain_table_size = sizeof(cruiserhmain2_dcg_ratio_table_10bit),
		},
		.multi_exposure_ana_gain_range[IMGSENSOR_EXPOSURE_LE].min = BASEGAIN * 4,
		.multi_exposure_ana_gain_range[IMGSENSOR_EXPOSURE_ME].min = BASEGAIN * 1,
		.multi_exposure_ana_gain_range[IMGSENSOR_EXPOSURE_LE].max = BASEGAIN * 64,
	...
	{// mode 13 G11_RSS_Crop_2048x1536_modeh_1536Msps_pd_type2_30.085470fps_cphy_3trio_10bit_valid_disc_20251105_V0.2.ini
		...
		.mode_setting_table = cruiserhmain2_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(cruiserhmain2_custom9_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = CRUISERHMAIN2_SENSOR_ID,
	...
	.ana_gain_type = 4,
	.ana_gain_step = 1,
	.ana_gain_table = cruiserhmain2_ana_gain_table,
	.ana_gain_table_size = sizeof(cruiserhmain2_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 6,
	...
	.g_cali = cruiserhmain2_get_sensor_cali,
	.s_gph = set_group_hold,
	.s_cali = cruiserhmain2_set_sensor_cali,
	...
	.init_setting_table = cruiserhmain2_init_setting,
	.init_setting_len = ARRAY_SIZE(cruiserhmain2_init_setting),
	.mode = mode_struct,
...
struct subdrv_entry cruiserhmain2_mipi_raw_entry = {
	.name = "cruiserhmain2_mipi_raw",
	.id = CRUISERHMAIN2_SENSOR_ID,
	.pw_seq = pw_seq,
...
static unsigned int read_cruiserhmain2_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != cruiserhmain2_eeprom_info[meta_id].meta)
		return -1;

	if (size != cruiserhmain2_eeprom_info[meta_id].size)
		return -1;

	addr = cruiserhmain2_eeprom_info[meta_id].start;
	readsize = cruiserhmain2_eeprom_info[meta_id].size;
...
static int cruiserhmain2_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
    ...
    if (ae_ctrl) {
        ...
        cruiserhmain2_set_shutter(ctx, (u8*)&ae_ctrl[SHUTTER_NE_FRM_1], NULL);
        cruiserhmain2_set_gain(ctx, (u8*)&ae_ctrl[GAIN_NE_FRM_1], NULL);

        cruiserhmain2_seamless_end(ctx, ctx->frame_length, ae_ctrl[SHUTTER_NE_FRM_1],
        cruiserhmain2_get_tline_ns(ctx, ctx->line_length, ctx->pclk));
    }
    // set_group_hold(ctx, 0);
    else {
        cruiserhmain2_set_shutter(ctx, (u8*)&ctx->exposure[0], NULL);
        cruiserhmain2_set_gain(ctx, (u8*)&ctx->ana_gain[0], NULL);

        cruiserhmain2_seamless_end(ctx, ctx->frame_length, ctx->exposure[0],
        cruiserhmain2_get_tline_ns(ctx, ctx->line_length, ctx->pclk));
	}
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, CRUISERHMAIN2_EEPROM_ADDR >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, CRUISERHMAIN2_EEPROM_ADDR >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, CRUISERHMAIN2_EEPROM_ADDR >> 1, reg, (CRUISERHMAIN2_EEPROM_ADDR & 0xFE) | 0x01);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, CRUISERHMAIN2_EEPROM_ADDR >> 1, reg, CRUISERHMAIN2_EEPROM_ADDR & 0xFE);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if (((pStereodata->uSensorId == CRUISERHMAIN2_SENSOR_ID))
            && (data_length == CALI_DATA_SLAVE_LENGTH)
            && (data_base == CRUISERHMAIN2_STEREO_MW_START_ADDR)) {
        ...
        } else if (((pStereodata->uSensorId == CRUISERHMAIN2_SENSOR_ID))
            && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == CRUISERHMAIN2_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN2_AESYNC_START_ADDR),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN2_AESYNC_START_ADDR+1),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN2_AESYNC_START_ADDR+2),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN2_AESYNC_START_ADDR+3),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN2_AESYNC_START_ADDR+4),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN2_AESYNC_START_ADDR+5),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN2_AESYNC_START_ADDR+6),
                    read_cmos_eeprom_8(ctx, CRUISERHMAIN2_AESYNC_START_ADDR+7));
        ...
        LOG_INF("cruiserhmain2 write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int cruiserhmain2_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	read_cruiserhmain2_eeprom_info(ctx, EEPROM_META_STEREO_MW_MAIN_DATA,
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, CRUISERHMAIN2_EEPROM_ADDR >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "cruiserhmain2 read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, sizeof(otp_data_checksum));
	DRV_LOGE(ctx, "cruiserhmain2 read_otp_info end\n");
}
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			*sensor_id = (subdrv_i2c_rd_u8(ctx, addr_h) << 8) |
			...
			if (*sensor_id == CRUISERHMAIN2_IMGSENSOR_ID) {
...
static int cruiserhmain2_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
 ...
 //      cruiserhmain2_feedback_awbgain(ctx, r_gain, b_gain);
 //   } else {
...
static int cruiserhmain2_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
		...
		if (*(u32 *)para)
			cruiserhmain2_set_shutter(ctx, para, len);
...
static int open(struct subdrv_ctx *ctx)
{
	...
	cruiserhmain2_i2c_burst_wr_regs_u8(ctx, ctx->s_ctx.init_setting_table, ctx->s_ctx.init_setting_len);

	/* mirror flip*/
	cruiserhmain2_set_mirror_flip(ctx, 3);
	/* PDC setting */
...
void cruiserhmain2_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
...
int cruiserhmain2_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64 *feature_data = (u64 *) para;
	cruiserhmain2_get_min_shutter_by_scenario(ctx,
...
{
	if(exp_cnt == 1) {
		cruiserhmain2_set_shutter_frame_length_convert(ctx, shutters[0], frame_length);
		return ;
	}
}
...
static int cruiserhmain2_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	cruiserhmain2_set_multi_shutter_frame_length(ctx, (u64 *)(*feature_data),
		(u64) (*(feature_data + 1)), (u64) (*(feature_data + 2)));
	return 0;
}

static void cruiserhmain2_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt)
{
	int i = 0;
	u64 values[3] = {0};
	...
	cruiserhmain2_set_multi_shutter_frame_length(ctx, values, exp_cnt, 0);
}
...
static int cruiserhmain2_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	cruiserhmain2_set_hdr_tri_shutter(ctx, feature_data, 2);
	return 0;
}
...
static int cruiserhmain2_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	cruiserhmain2_set_hdr_tri_shutter(ctx, feature_data, 3);
	return 0;
}
...
static void cruiserhmain2_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length)
{
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps >= 297 && realtime_fps <= 305)
			cruiserhmain2_set_max_framerate(ctx, 296, 0);
		else if (realtime_fps >= 147 && realtime_fps <= 150)
			cruiserhmain2_set_max_framerate(ctx, 146, 0);
		else {
...
static int cruiserhmain2_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	cruiserhmain2_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], ((u64*)para)[1]);
	return 0;
}
...
void cruiserhmain2_set_dummy(struct subdrv_ctx *ctx)
{
}
...
// 		if (ctx->s_ctx.sensor_id == CRUISERHMAIN2_SENSOR_ID) {
