	...
	int *check_result)
{
	...
	case ES4G_PREEMPT_POLICY_HOK:
		if (task_specific_type(get_pipeline_sched_prop(p), ES4G_TASK_PROP_PERIODIC_AND_CRITICAL)) {
			*check_result = 1;
		}
		break;

	case ES4G_PREEMPT_POLICY_GENSHIN:
		if (index >= MAX_KEY_THREAD_RECORD) {
			break;
		}
		if (task_specific_type(get_pipeline_sched_prop(p), ES4G_TASK_PROP_PIPELINE)) {
			curr = rq->curr;
			if (curr && task_is_fair(curr)) {
				*check_result = 1;
			}
		}
		break;
