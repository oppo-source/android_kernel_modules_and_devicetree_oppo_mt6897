...
static struct iris_mode_info *panel_tm_arry;
...
void iris_set_panel_timing(uint32_t index, const struct iris_mode_info *timing)
{
	...
	memcpy(&panel_tm_arry[index], timing, sizeof(struct iris_mode_info));
}
#endif

void iris_init_panel_timing(void *dev)
{
	struct drm_connector *conn = (struct drm_connector *)dev;
...
static void _iris_init_param(struct device_node *np)
{
	...
	if (panel_tm_num > 1) {
		...
		u32 buf_size = panel_tm_num * sizeof(struct iris_mode_info);
...
static uint32_t _iris_get_timing_index(const struct iris_mode_info *timing)
{
	...
	for (i = 0; i < panel_tm_num; i++) {
		struct iris_mode_info *t = &panel_tm_arry[i];
...
static uint32_t _iris_generate_switch_case(
		const struct iris_mode_info *new_timing)
{
...
void iris_update_last_pt_timing(const struct iris_mode_info *pt_timing)
{
	if (!_iris_support_timing_switch())
		return;
...
static void _iris_sync_dtg(void)
{
	...
	// update DTG TE_SEL setting
	dynamic_switch_opt[dynamic_opt_index].ip = IRIS_IP_DTG;
	dynamic_switch_opt[dynamic_opt_index].opt_id = 0xF1;
	dynamic_switch_opt[dynamic_opt_index].chain = 0x01;
	dynamic_opt_index++;
...
{
	struct iris_mode_info *last_pt_timing = &panel_tm_arry[last_pt_tm_index];
...
static struct drm_display_mode *iris_scaling_mode_mapping(struct mtk_drm_crtc *mtk_crtc,
												int id)
{
	unsigned int i = 0;
	int fps = 0;
	struct drm_display_mode *map_mode = NULL;
	struct drm_display_mode *m = NULL;
	struct drm_display_mode *base_mode = mtk_drm_crtc_avail_disp_mode(&mtk_crtc->base, id);

	if (!base_mode) {
		IRIS_LOGE("%s: invalid input parameters", __func__);
		return NULL;
	}

	fps = drm_mode_vrefresh(base_mode);
	map_mode = mtk_drm_crtc_avail_disp_mode(&mtk_crtc->base,
					mtk_crtc->scaling_ctx.mode_mapping[id]);
	if (!map_mode) {
		IRIS_LOGE("%s: failed to get mapped mode", __func__);
		return NULL;
	}

	IRIS_LOGI("%s fps:%d map", __func__, fps);
	for (i = 0; i < mtk_crtc->avail_modes_num; i++) {
		m = mtk_drm_crtc_avail_disp_mode(&mtk_crtc->base, i);
		if (!m)
			continue;
		if ((drm_mode_vrefresh(m)) == fps && (map_mode->vdisplay == m->vdisplay)) {
			map_mode = m;
			break;
		}
	}
...
int iris_switch_mtk(void *handle, void *dev, int id)
{
	...
	if(dev) {
		...
		if (mtk_crtc->scaling_ctx.cust_mode_mapping)
			dsp_mode = iris_scaling_mode_mapping(mtk_crtc, id);
		else
			dsp_mode = mtk_drm_crtc_avail_disp_mode(&mtk_crtc->base, id);
		if (!dsp_mode)
			return ret;
		iris_get_vendor_cfg()->dsp_mode = dsp_mode;
		iris_sync_timing(&new_timing, dsp_mode);
		iris_pre_switch(&new_timing);
		iris_sync_cur_timing();
		ret = iris_switch_opt(handle, NULL, &new_timing);
	}
	return ret;
}
...
{
	int ret = -EINVAL;
	bool cur_mode = iris_is_pt_mode(false);
	if (cur_mode)
		iris_vdo_mode_send_cmd_pre(handle);
	ret = iris_switch_mtk(handle, dev, id);
	if (cur_mode)
...
{
	return iris_switch_mtk(handle, dev, id);
}
