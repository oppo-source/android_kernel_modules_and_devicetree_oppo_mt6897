...
int iris_debug_display_info_get(char *kbuf, int size)
{
	...
	len += scnprintf(kbuf + len, size - len,
	...
	if (comp) {
		...
		len += scnprintf(kbuf + len, size - len,
				...
				(mtk_crtc_is_frame_trigger_mode(crtc) ?
				"command" : "video"),
				(iris_check_2nd_dsc_enable() ? "DSC" : "RAW"));
