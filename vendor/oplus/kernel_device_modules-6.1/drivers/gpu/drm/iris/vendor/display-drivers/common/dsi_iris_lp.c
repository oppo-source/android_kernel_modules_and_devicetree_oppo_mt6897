...
int iris_panel_read_status(int i, unsigned char cmd)
{
	...
	struct iris_cmd_desc cmds = {
		{0, MIPI_DSI_DCS_READ, MIPI_DSI_MSG_REQ_ACK,
		sizeof(get_esd_state), get_esd_state, sizeof(read_cmd_rbuf), read_cmd_rbuf},
		1, 0};

	struct iris_cmd_set cmdset = {
		.state = IRIS_CMD_SET_STATE_LP,
		.count = 1,
		.cmds = &cmds,
	};
	...
	if ((i >= 0) && (i < ESD_CHK_NUM)) {
		ret = iris_pt_send_panel_cmd(&cmdset);

		if (iris_is_pt_mode(false))
			pcfg_ven->esd_chk_val[i] = read_cmd_rbuf[0];
		else {
			IRIS_LOGD("[%s][%d] is not in pt mode.", __func__, i);
			pcfg_ven->esd_chk_val[i] = ESD_CHK_EXIT;
			ret = 1;
		}
		IRIS_LOGD("[%s]esd_chk_val[%d]: 0x%02x", __func__, i, pcfg_ven->esd_chk_val[i]);
	} else
		IRIS_LOGE("[%s] index error.", __func__);
...
int iris_read_status(int i, unsigned char cmd)
{
	...
	if ((pcfg_ven->esd_check_num > 0) && (pcfg_ven->esd_read_index >= pcfg_ven->esd_check_num))
		pcfg_ven->esd_read_index = 0;
	IRIS_LOGV("%s() esd_check_num = %d, read_index = %d, read_flag = %d", __func__,
		pcfg_ven->esd_check_num, pcfg_ven->esd_read_index, pcfg_ven->esd_read_flag);
		...
		if (iris_esd_ctrl_get() & 0x1) {
			...
			if (rc <= 0) {
				if (iris_esd_ctrl_get() & 0x4)
					pcfg_ven->esd_chk_val[0] = 0xbad; //trigger esd recovery
				goto exit;
			} else {
				/* reset the esd_chk_val */
				pcfg_ven->esd_chk_val[0] =
					pcfg_ven->panel_ext->params->lcm_esd_check_table[0].para_list[0];
			}
		}
	}
	...
	if (iris_esd_ctrl_get() & 0x2) {
		if (pcfg->pwil_mode != FRC_MODE) {
			if ((i == pcfg_ven->esd_read_index) && !pcfg_ven->esd_read_flag) {
				rc = iris_panel_read_status(i, cmd);
				pcfg_ven->esd_read_flag = true;

				if (pcfg_ven->esd_check_num > 0)
					pcfg_ven->esd_read_index++;
			}
		}
...
void iris_set_esd_check_ongoing(bool status)
{
	...
	pcfg_ven->is_esd_check_ongoing = status;
}
...
bool iris_get_esd_check_ongoing(void)
{
	...
	return pcfg_ven->is_esd_check_ongoing;
}
...
#define CHECK_KICKOFF_FPS_DURATION 1 /*EVERY 1s*/
...
void _iris_disable_temp_sensor(void)
{
	...
	if (pcfg->iris_i2c_write(0xf0010030, 1) < 0)
		IRIS_LOGE("i2c disable tempsensor failed, line:%d", __LINE__);
	if (pcfg->iris_i2c_write(0xf0000150, 3) < 0)
		IRIS_LOGE("i2c disable tempsensor failed, line:%d", __LINE__);
	if (pcfg->iris_i2c_write(0xf0000150, 0) < 0)
		IRIS_LOGE("i2c disable tempsensor failed, line:%d", __LINE__);
	if (pcfg->iris_i2c_write(0xf0010030, 0) < 0)
		IRIS_LOGE("i2c disable tempsensor failed, line:%d", __LINE__);
}
...
void _iris_bulksram_power_domain_proc(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		rc = pcfg->iris_i2c_write(0xf0000068, 0x43);
		if (rc < 0) {
...
int getFrameDiff(long timeDiff)
{
	...
	if (pcfg->panel_ext) {
		panel_rate = pcfg->timing.refresh_rate;
	}
	if (panel_rate == 90)
...
#if defined(CHECK_KICKOFF_FPS_CADNENCE)
	if (iris_aux_dsi(comp)) {
		iris_check_kickoff_fps_cadence_2nd();
		return 0;
	}
	if (iris_main_dsi(comp))
...
static int _iris_esd_check(void)
{
	...
	if (pcfg->ocp_read_by_i2c && pcfg->iris_i2c_read) {
		...
		rc = iris_dsi_send_cmds(cmdset.cmds, cmdset.count,
...
void iris_dump_status(void)
{
	...
	IRIS_LOGI("ULPS - enabled:%d, lp:%d",
			/*pcfg->display->panel->ulps_feature_enabled*/0, pcfg->lp_ctrl.ulps_lp);
	...
	for (i = 0; i < len; i++) {
		if (read_path == PATH_I2C) { //use i2c to read
			rc = pcfg->iris_i2c_read(_regs_dump[i], &value);
			if (rc != 0) {
