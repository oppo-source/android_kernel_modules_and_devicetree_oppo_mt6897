...
static void iris_dtg_setting_update(void *handle, bool lock, bool i2c_path)
{
	...
	u32 sys_array_120[][2] = {
		{0xF000004C, 0x06673310},
		{0xF0000048, 0x00012490},
		{0xF0010040, 0x00000020},
	};
	u32 sys_array_144[][2] = {
		{0xF000004C, 0x03373310},
		{0xF0000048, 0x0002D490},
		{0xF0010040, 0x00000020},
		{0xF0000160, 0x00000010},
		{IRIS_DTG_ADDR + DTG_HFP, 0x54},
		{IRIS_DTG_ADDR + DTG_VFP, 0x74},
		{IRIS_DTG_ADDR + DTG_OVS_DLY, 0x41},
		{IRIS_DTG_ADDR + DTG_FI_PRELOAD_DLY, 0xB32},
		{IRIS_DTG_ADDR + DTG_UPDATE, 0x2},
	};
	...
	switch (pcfg->ap_te) {
		case 144:
			if (!is_project_casio_m()) {
				...
				for (int i = 0;i < ARRAY_SIZE(sys_array_144); i++) {
					cmd[idx++] = sys_array_144[i][0];
					cmd[idx++] = sys_array_144[i][1];
				}
				if (!i2c_path) {
						cmd[idx++] = pcfg->iris_mipi_tx_header_addr;
						cmd[idx++] = 0x5000639;
		...
		case 120:
			...
			if (is_project_casio_m()) {
				for (int i = 0; i < ARRAY_SIZE(sys_array_120); i++) {
					cmd[idx++] = sys_array_120[i][0];
					cmd[idx++] = sys_array_120[i][1];
				}
				cmd[idx++] = 0xF0000160;
				cmd[idx++] = pcfg->mipi_hopping_status ? 0x11 : 0x10;
