...
{
	int32_t rc = 0;
	u8 *key = "pxlw,ocp_write_by_i2c";
	struct iris_dts_ops *p_dts_ops = iris_get_dts_ops();

	if (!p_dts_ops)
		return -EINVAL;

	rc = p_dts_ops->read_u32(np, key, &(pcfg->ocp_write_by_i2c));
	if (rc) {
		IRIS_LOGW("%s(), failed to %s, return: %d",
			__func__, key, rc);
		pcfg->ocp_write_by_i2c = 0;
	}
...
int pw_dbgfs_cont_splash_init(void *display)
{
	...
	debugfs_create_u32("ocp_write_by_i2c", 0644, pcfg->dbg_root, &pcfg->ocp_write_by_i2c);
...
bool is_project_casio_m(void)
{
	struct iris_cfg *pcfg = iris_get_cfg();

	if (pcfg == NULL || pcfg->panel_name == NULL)
		return false;

	if (!strncmp(pcfg->panel_name, "oplus25711_nt37706a_fhdp_dsi_vdo_144hz_dphy_boe", 47))
		return true;
	else
		return false;
}
