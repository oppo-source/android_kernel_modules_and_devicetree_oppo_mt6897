...
static int _iris_configure(u32 display, u32 type, u32 value)
{
	...
	case IRIS_CM_COLOR_GAMUT:
		//if (value > 15)
		//	goto error;
		// after speed up light sequence, precsc is always disabled.
		dpp_precsc_enable = 0;
	...
	case IRIS_ANALOG_BYPASS_MODE:
	{
		...
		if ((pcfg->rx_mode == IRIS_VIDEO_MODE) && (value == ANALOG_BYPASS_MODE))
			lock = false;
...
int iris_configure_get_i7p(u32 display, u32 type, u32 count, u32 *values)
{
	...
	case IRIS_DUMP_APL_PER_FRAME:
		...
		*values = 0;
		...
		pcfg->lightup_ops.release_panel_lock();
		break;
	case IRIS_GET_CMDQ_EMPTY:
		if (pcfg->cmdq_empty)
			*values = pcfg->cmdq_empty();
		break;
	default:
		return -EFAULT;
	}
