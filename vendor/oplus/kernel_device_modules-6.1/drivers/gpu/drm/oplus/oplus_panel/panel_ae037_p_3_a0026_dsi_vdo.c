...
static const struct drm_display_mode disp_mode_60Hz = {
		.clock = 517544,
		.hdisplay = 1280,
		.hsync_start = 1280 + 157,/* HFP */
		.hsync_end = 1280 + 157 + 4,/* HSA */
		.htotal = 1280 + 157 + 4 + 20,/* HBP */
		.vdisplay = 2800,
...
static const struct drm_display_mode disp_mode_144Hz = {
		.clock = 576977,
		.hdisplay = 1280,
		.hsync_start = 1280 + 57,/* HFP */
		.hsync_end = 1280 + 57 + 4,/* HSA */
		.htotal = 1280 + 57 + 4 + 20,/* HBP */
		.vdisplay = 2800,
...
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	dsi_set_backlight[0].para_list[1] = level >> 8;
	dsi_set_backlight[0].para_list[2] = level & 0xFF;
...
/*
static int oplus_lcm_set_hbm(void *dsi, dcs_write_gce_pack cb,
		void *handle, unsigned int hbm_mode)
{
...
static int lcm_get_modes(struct drm_panel *panel, struct drm_connector *connector)
{
	...
	if (!mode[3]) {
		pr_err("failed to add mode %ux%ux@%u\n", disp_mode_144Hz.hdisplay, disp_mode_144Hz.vdisplay, drm_mode_vrefresh(&disp_mode_144Hz));
		return -ENOMEM;
	}
