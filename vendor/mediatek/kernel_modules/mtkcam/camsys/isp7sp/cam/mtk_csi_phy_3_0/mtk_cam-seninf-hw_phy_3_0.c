...
static int csirx_phyA_setting(struct seninf_ctx *ctx)
{
	...
	if (!ctx->is_cphy) { //Dphy
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
				if (ctx->port == CSI_PORT_4 || ctx->port == CSI_PORT_2) {
					prj_id = get_project();
					if ((24749 == prj_id)) {
						SENINF_BITS(baseA, CDPHY_RX_ANA_5,
								RG_CSI0_CDPHY_EQ_BW, 0x3);
						dev_info(ctx->dev,"caymanb front uwide project: %d port: %d", prj_id, ctx->port);
					}
				} else {
					SENINF_BITS(baseA, CDPHY_RX_ANA_5,
		...
		if (ctx->is_4d1c) {
			...
			if (ctx->port == CSI_PORT_3) {
				prj_id = get_project();
				if ((24749 == prj_id)) {
					dev_info(ctx->dev, "caymanb main sensor: csi_port = %d\n", ctx->port);
					SENINF_BITS(base, CDPHY_RX_ANA_5,
								RG_CSI0_CDPHY_EQ_BW, 0x3);
					SENINF_BITS(baseB, CDPHY_RX_ANA_5,
								RG_CSI0_CDPHY_EQ_BW, 0x3);
