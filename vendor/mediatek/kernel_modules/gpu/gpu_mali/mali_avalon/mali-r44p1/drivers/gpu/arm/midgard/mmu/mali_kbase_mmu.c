					...
					enum kbase_mmu_op_type flush_op)
{
	...
	if (nr > 512*512)						// ~1G
		dev_warn(kbdev->dev, "%s: vpfn(%#llx), nr(0x%zx)", __func__, vpfn, nr);
	if (nr > 512*512*512) {					// if nr is unexcepted(too big, ~500G), limit it.
		dev_warn(kbdev->dev, "%s: nr(0x%zx) is too big, limit it.", __func__, nr);
		nr = 512*512*512;
	}
	u64 dump_count = 0;
	...
	while (nr) {
		...
		for (level = MIDGARD_MMU_TOPLEVEL;
			...
			if (mmu_mode->ate_is_valid(page[index], level))
				...
				dump_count++;
				if ((dump_count < 200)
					|| ((dump_count < 10000) && (dump_count % 100 == 0))			// print per 100
					|| ((dump_count < 1000000) && (dump_count % 10000 == 0))		// print per 1,0000
				) {
					dev_warn(kbdev->dev, "Invalid PTE found @ level %d for VA %llx; dump_count(%#llx)",
						level, vpfn << PAGE_SHIFT, dump_count);
				}
...
out:
	if (dump_count > 200)
		dev_warn(kbdev->dev, "%s: Invalid PTE found (%#llx) times.", __func__, dump_count);
	return 0;
}
