...
const uint32_t TXS_BMC_PENDING_CNT_LIMIT[ENUM_PKT_FLAG_NUM] = {
	[ENUM_PKT_ARP] = 10,
};
...
void nicTxInitPktPID(struct ADAPTER *prAdapter, uint8_t ucWlanIndex)
{
	...
	for (i = 0; i < ENUM_PKT_FLAG_NUM; i++) {
		...
		prAdapter->u4BMCPktPIDPendingCnt[ucWlanIndex][i] = 0;
	}
}
...
static inline bool nicTxBMCPktPIDCntIsLimited(struct ADAPTER *prAdapter,
	struct MSDU_INFO *prMsduInfo)
{
	uint8_t ucWlanIndex = prMsduInfo->ucWlanIndex;
	uint8_t ucPktType = prMsduInfo->ucPktType;
	...
	if (prAdapter->u4BMCPktPIDPendingCnt[ucWlanIndex][ucPktType]
		<= TXS_BMC_PENDING_CNT_LIMIT[ucPktType]) {
		GLUE_INC_REF_CNT(prAdapter->u4BMCPktPIDPendingCnt[
			ucWlanIndex][ucPktType]);
		return FALSE;
	}
			...
			struct MSDU_INFO *prMsduInfo)
{
	...
	if (nicTxPktPIDIsLimited(prAdapter, prMsduInfo) ||
	    nicTxBMCPktPIDCntIsLimited(prAdapter, prMsduInfo)) {
...
/* fos_change begin */
	...
	do {
		...
		if (prMsduInfo->ucPktType == ENUM_PKT_ARP &&
		    ...
		    prAdapter->u4BMCPktPIDPendingCnt[
			prMsduInfo->ucWlanIndex][ENUM_PKT_ARP] > 0)
			GLUE_DEC_REF_CNT(prAdapter->u4BMCPktPIDPendingCnt[
				prMsduInfo->ucWlanIndex][ENUM_PKT_ARP]);
#endif
