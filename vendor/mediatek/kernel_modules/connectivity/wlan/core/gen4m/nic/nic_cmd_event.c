	...
	uint8_t ucStaRecIdx)
{
	...
	if (prEvent->u4Flags & BIT(0)) {
		...
		DBGLOG(NIC, TRACE, "sta[%u] trans(%u):%u fail:%u ls:%u\n",
			ucStaRecIdx,
			OFFSET_OF(struct EVENT_STA_STATISTICS, u4TransmitCount),
			prEvent->u4TransmitCount,
			prEvent->u4TransmitFailCount,
			prEvent->u2LinkSpeed);
		...
		DBGLOG(NIC, TRACE,
			"txv size:%u ofset:%u B0=[0x%x, 0x%x, 0x%x]\n",
			sizeof(prEvent->rTxVector),
			OFFSET_OF(struct EVENT_STA_STATISTICS, rTxVector),
			prEvent->rTxVector[0].u4TxV[0],
			prEvent->rTxVector[0].u4TxV[1],
			prEvent->rTxVector[0].u4TxV[2]);
			      ...
			      struct WIFI_EVENT *prEvent)
{
	...
	prBssInfo = p2pGetAisConnectedBss(prAdapter);
	if (!prBssInfo)
