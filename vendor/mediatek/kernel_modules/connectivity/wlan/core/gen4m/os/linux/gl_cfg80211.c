		      ...
		      struct cfg80211_scan_request *request)
{
	...
	for (i = 0; i < request->n_channels; i++) {
		...
		nicFreq2ChannelNum(request->channels[i]->center_freq *
								1000);
		if (u4channel == 0) {
			DBGLOG(REQ, WARN, "Wrong Channel freq=%u\n",
				request->channels[i]->center_freq);
			continue;
		}
		if (ucChIdx >= MAXIMUM_OPERATION_CHANNEL_LIST) {
			DBGLOG(REQ, INFO,
		...
		prScanRequest->arChannel[ucChIdx].ucChannelNum = u4channel;
		...
		case KAL_BAND_2GHZ:
			...
			if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.
				...
				continue;
#endif
			prScanRequest->arChannel[ucChIdx].eBand = BAND_2G4;
			break;
		case KAL_BAND_5GHZ:
#if (CFG_SUPPORT_WIFI_6G == 1)
			if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.
				...
				continue;
#endif
			prScanRequest->arChannel[ucChIdx].eBand = BAND_5G;
			break;
#if (CFG_SUPPORT_WIFI_6G == 1)
		...
		case KAL_BAND_6GHZ:
			...
			prScanRequest->arChannel[ucChIdx].eBand = BAND_6G;
			break;
#endif
		default:
			DBGLOG(REQ, WARN, "UNKNOWN Band %d(chnl=%u)\n",
				   ...
				   u4channel);
			prScanRequest->arChannel[ucChIdx].eBand = BAND_NULL;
			break;
		}
		ucChIdx++;
	}
	prScanRequest->u4ChannelNum = ucChIdx;
